...
d1demo <- read.csv("../data/study1/d1_demo.csv") 
...
d1 <- read.csv("../data/study1/d1_data.csv") #updated from "Study2.csv"
...
# d2c <- read.csv("../data/d2c.csv") # repreprocessed data, identical to above
...
# lmr3cspb.OK <- lmer(causation.response ~ (norm * outcome) + (knowledge * norm) + (knowledge+outcome|scenario) + (1|ResponseId), data=d3.cspb,
#                     control = lmerControl(optimizer ='optimx', optCtrl=list(method='L-BFGS-B')))
...
We also probed interaction effects and did not find any interaction effects, including the three-way *Knowledge* $\times$ *Harm Type* $\times$ *Outcome* interaction, $\chi^2(1)$ = `r round(lmr3cspb.3way$Chisq[2],digits=2)`, *p* = `r max(.001,round(lmr3cspb.3way$'Pr(>Chisq)'[2],digits=3))`, and the two-way interactions of *Knowledge* $\times$ *Outcome*, $\chi^2(1)$ = `r round(lmr3cspb.OxK$Chisq[2],digits=2)`, *p* `r round(lmr3cspb.OxK$'Pr(>Chisq)'[2],digits=3)`. [[[[[[This interaction suggests that in probable outcomes (*t*(`r round(d3a.empairOK$df[6])`) = `r round(d3a.empairOK$t.ratio[6], digits=2)`, *p* = `r round(d3a.empairOK$p.value[6], digits=3)`), participants' causal judgments did not differentiate much between agents that knowingly did harm (*M* = `r round(d3a.emsumOK$emmean[4], digits=2)`, 95% CI = [`r round(d3a.emsumOK$lower.CL[4],digits=2)`, `r sprintf("%.2f", d3a.emsumOK$upper.CL[4])`]) and ignorant agents (*M* = `r round(d3a.emsumOK$emmean[3], digits=2)`, 95% CI = [`r round(d3a.emsumOK$lower.CL[3],digits=2)`, `r round(d3a.emsumOK$upper.CL[3],digits=2)`]). Whereas in improbable outcomes (*t*(`r round(d3a.empairOK$df[1])`) = `r round(d3a.empairOK$t.ratio[1], digits=2)`, *p* = `r round(d3a.empairOK$p.value[1], digits=3)`), the difference between agents of different knowledge status was salient, with participants assigning more causation to knowledgeable agents (*M* = `r round(d3a.emsumOK$emmean[2], digits=2)`, 95% CI = [`r round(d3a.emsumOK$lower.CL[2],digits=2)`, `r round(d3a.emsumOK$upper.CL[2],digits=2)`]) than ignorant agents (*M* = `r round(d3a.emsumOK$emmean[1], digits=2)`, 95% CI = [`r round(d3a.emsumOK$lower.CL[1],digits=2)`, `r round(d3a.emsumOK$upper.CL[1],digits=2)`]).]]]   and the two two-way interactions of *Knowledge* $\times$ *Harm Type*, $\chi^2(1)$ = `r round(lmr3cspb.NxK$Chisq[2],digits=2)`, *p* = `r round(lmr3cspb.NxK$'Pr(>Chisq)'[2],digits=3)`, and *Outcome* $\times$ *Harm Type*, $\chi^2(1)$ = `r round(lmr3cspb.NxO$Chisq[2], digits=2)`, *p* = `r round(lmr3cspb.NxO$'Pr(>Chisq)'[2],digits=3)`.
...
Mirroring Experiment 3b, we once again asked participants to provide their ratings of outcome-valence and outcome-normality, but this time returned to the scenarios from Experiments 1-2, where we intended to manipulate the valence of the outcome. While the prior ratings allowed us to demonstrate that outcome-normality can matter on its own, these additional ratings allow us to ask the further question of whether the original interaction effect we observed in Experiment 2 may in fact have arisen because of perceived differences in the normality of the outcome rather than perceived differences in the valence of the outcome.
...
# Experiment 4a: Modality and counterfactuality judgments in cases of good/bad outcomes and norm violations
...
## Md main effects (knowledge significant <.001, outcome significant .004, norm insignificant; used knowledge+outcome for convergence)
...
# d4gbmd.emsumO <- summary(emmeans::emmeans(lm4gb_md.0, ~ outcome, pbkrtest.limit = 3242)) 
# saveRDS(d4gbmd.emsumO,"models/d4gbmd.emsumO.rds")
d4gbmd.emsumO <- readRDS("models/d4gbmd.emsumO.rds")
# d4gbmd.empairO <- summary(pairs(emmeans(lm4gb_md.0, ~ outcome, pbkrtest.limit = 3242)))
# saveRDS(d4gbmd.empairO,"models/d4gbmd.empairO.rds")
d4gbmd.empairO <- readRDS("models/d4gbmd.empairO.rds")

# d4gbmd.emsumK <- summary(emmeans::emmeans(lm4gb_md.0, ~ knowledge, pbkrtest.limit = 3242))
# saveRDS(d4gbmd.emsumK,"models/d4gbmd.emsumK.rds")
d4gbmd.emsumK <- readRDS("models/d4gbmd.emsumK.rds")
# d4gbmd.empairK <- summary(pairs(emmeans(lm4gb_md.0, ~ knowledge, pbkrtest.limit = 3242)))
...
## Cf: Outcome x Knowledge interaction (significant 0.01003)
...
# d4gbcf.emsumO <- summary(emmeans::emmeans(lm4gb_cf.0, ~ outcome, pbkrtest.limit = 3273))
# saveRDS(d4gbcf.emsumO,"models/d4gbcf.emsumO.rds")
d4gbcf.emsumO <- readRDS("models/d4gbcf.emsumO.rds")
# d4gbcf.empairO <- summary(pairs(emmeans(lm4gb_cf.0, ~ outcome, pbkrtest.limit = 3273)))
# saveRDS(d4gbcf.empairO,"models/d4gbcf.empairO.rds")
d4gbcf.empairO <- readRDS("models/d4gbcf.empairO.rds")

# d4gbcf.emsumOK <- summary(emmeans::emmeans(lm4gb_cf.0, ~ outcome*knowledge, pbkrtest.limit = 3273))
# saveRDS(d4gbcf.emsumOK,"models/d4gbcf.emsumOK.rds")
d4gbcf.emsumOK <- readRDS("models/d4gbcf.emsumOK.rds")
# d4gbcf.empairOK <- summary(emmeans::emmeans(lm4gb_cf.0, pairwise ~ outcome*knowledge, pbkrtest.limit = 3273))
...
## three-way interaction (insignificant .073)
...
# two-way: Outcome x Knowledge interaction (significant, 0.013)
...
# d4gb_pot.emsumO <- summary(emmeans::emmeans(lmr4gbpot.i0, ~ outcome, pbkrtest.limit = 3239))
# saveRDS(d4gb_pot.emsumO,"models/d4gb_pot.emsumO.rds")
d4gb_pot.emsumO <- readRDS("models/d4gb_pot.emsumO.rds")
# d4gb_pot.empairO <- summary(pairs(emmeans(lmr4gbpot.i0, ~ outcome, pbkrtest.limit = 3239)))
# saveRDS(d4gb_pot.empairO,"models/d4gb_pot.empairO.rds")
d4gb_pot.empairO <- readRDS("models/d4gb_pot.empairO.rds")

# d4gb_pot.emsumK <- summary(emmeans::emmeans(lmr4gbpot.i0, ~ knowledge, pbkrtest.limit = 3239))
# saveRDS(d4gb_pot.emsumK,"models/d4gb_pot.emsumK.rds")
d4gb_pot.emsumK <- readRDS("models/d4gb_pot.emsumK.rds")
# d4gb_pot.empairK <- summary(pairs(emmeans(lmr4gbpot.i0, ~ knowledge, pbkrtest.limit = 3239)))
# saveRDS(d4gb_pot.empairK,"models/d4gb_pot.empairK.rds")
d4gb_pot.empairK <- readRDS("models/d4gb_pot.empairK.rds")

# d4gb_pot.emsumOK <- summary(emmeans::emmeans(lmr4gbpot.i0, ~ outcome*knowledge, pbkrtest.limit = 3239))
# saveRDS(d4gb_pot.emsumOK,"models/d4gb_pot.emsumOK.rds")
d4gb_pot.emsumOK <- readRDS("models/d4gb_pot.emsumOK.rds")
# d4gb_pot.empairOK <- summary(emmeans::emmeans(lmr4gbpot.i0, pairwise ~ outcome*knowledge, pbkrtest.limit = 3239))
...
d4pb <- read.csv("../data/d4pb.csv", check.names = F) %>%
  mutate(`CF responses` = as.numeric(`CF responses`)) %>%
...
### Modality estimated marginal means
# d4pbmd.emsumK <- summary(emmeans::emmeans(lm4pb_md.0, ~ knowledge, pbkrtest.limit = 3326))
# saveRDS(d4pbmd.emsumK,"models/d4pbmd.emsumK.rds")
d4pbmd.emsumK <- readRDS("models/d4pbmd.emsumK.rds")
# d4pbmd.empairK <- summary(pairs(emmeans(lm4pb_md.0, ~ knowledge, pbkrtest.limit = 3326)))
# saveRDS(d4pbmd.empairK,"models/d4pbmd.empairK.rds")
d4pbmd.empairK <- readRDS("models/d4pbmd.empairK.rds")



## Cf main effects (knowledge significant .025, norm significant .05, outcome insignificant; used knowledge+outcome for convergence)
...
### Counterfactural estimated marginal means
# d4pbcf.emsumK <- summary(emmeans::emmeans(lm4pb_cf.0, ~ knowledge, pbkrtest.limit = 3327))
# saveRDS(d4pbcf.emsumK,"models/d4pbcf.emsumK.rds")
d4pbcf.emsumK <- readRDS("models/d4pbcf.emsumK.rds")
# d4pbcf.empairK <- summary(pairs(emmeans(lm4pb_cf.0, ~ knowledge, pbkrtest.limit = 3327)))
# saveRDS(d4pbcf.empairK,"models/d4pbcf.empairK.rds")
d4pbcf.empairK <- readRDS("models/d4pbcf.empairK.rds")

# d4pbcf.emsumN <- summary(emmeans::emmeans(lm4pb_cf.0, ~ norm, pbkrtest.limit = 3327))
...
## main effects (knowledge significant <.001, outcome and norm insignificant)
...
# d4pb_pot.emsumK <- summary(emmeans::emmeans(lmr4pbpot.i0, ~ knowledge, pbkrtest.limit = 3321))
# saveRDS(d4pb_pot.emsumK,"models/d4pb_pot.emsumK.rds")
d4pb_pot.emsumK <- readRDS("models/d4pb_pot.emsumK.rds")
# d4pb_pot.empairK <- summary(pairs(emmeans(lmr4pbpot.i0, ~ knowledge, pbkrtest.limit = 3321)))
