...
# NB: here, we do not analyze the pilot study with 3 scenarios (included in the folder /pilot_3_scenarios)
d1_demo <- read.csv("../data/d1_demo.csv") 

```

Since we did not have a priori assumptions, we collected data from `r length(unique(d1_demo$ResponseId))` participants recruited through Amazon Mechanical Turk (<http://www.mturk.com>) in Experiment 1. `r sum(d1_demo$SES != "")` participants (*M*~age~=`r round(mean(as.numeric(d1_demo$Age), na.rm=T),digits=2)`, *SD*~age~=`r round(sd(as.numeric(d1_demo$Age), na.rm=T),digits=2)`; `r sum(d1_demo$Gender == "Female", na.rm=T)` females) answered all our demographic questions.
...
d1 <- read.csv("../data/d1_data.csv")

d1$Agent <- factor(d1$Agent, levels=c("Object", "Ignorance", "Knowledge"))
d1$Outcome <- factor(d1$Outcome, levels=c("Good","Bad"))
d1$Cause <- as.numeric(d1$Cause)

### Interaction (significant: p <.001)
# lmr1.0 <- lmer(Cause ~ Agent * Outcome + (Agent*Outcome|Vignette) + (1|Subj), data=d1,
# control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
# lmr1.1 <- lmer(Cause ~ Agent + Outcome + (Agent*Outcome|Vignette) + (1|Subj), data=d1,
#                control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb'))) 
# lmr1.i <- anova(lmr1.0,lmr1.1)
# saveRDS(lmr1.i,"models/study1/lmr1i.rds")
lmr1.i <- readRDS("models/study1/lmr1i.rds")

### Main effect of agent (significant: p <.001)
# lmr1.2 <- lmer(Cause ~ Outcome + (Agent*Outcome|Vignette) + (1|Subj), data=d1,
#                control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
# lmr1.a <- anova(lmr1.1,lmr1.2)
# saveRDS(lmr1.a,"models/study1/lmr1a.rds")
lmr1.a <- readRDS("models/study1/lmr1a.rds")

### Main effect of outcome (significant: p = 0.00104)
# lmr1.3 <- lmer(Cause ~ Agent + (Agent*Outcome|Vignette) + (1|Subj), data=d1,
#                control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
# lmr1.o <- anova(lmr1.1,lmr1.3)
# saveRDS(lmr1.o,"models/study1/lmr1o.rds")
lmr1.o <- readRDS("models/study1/lmr1o.rds")
...
# saveRDS(d1.emsumAO,"models/study1/d1.emsumAO.rds")
d1.emsumAO <- readRDS("models/study1/d1.emsumAO.rds")
# d1.empairAO <- summary(emmeans::emmeans(lmr1.0, pairwise ~ Agent*Outcome))$contrasts
# saveRDS(d1.empairAO,"models/study1/d1.empairAO.rds")
d1.empairAO <- readRDS("models/study1/d1.empairAO.rds")

#Effect of Agent:
# d1.emsumA <- summary(emmeans::emmeans(lmr1.0, ~ Agent))
# saveRDS(d1.emsumA,"models/study1/d1.emsumA.rds")
d1.emsumA <- readRDS("models/study1/d1.emsumA.rds")
# d1.empairA <- summary(pairs(emmeans(lmr1.0, ~ Agent)))
# saveRDS(d1.empairA,"models/study1/d1.empairA.rds")
d1.empairA <- readRDS("models/study1/d1.empairA.rds")

#Effect of Outcome:
# d1.emsumO <- summary(emmeans::emmeans(lmr1.0, ~ Outcome))
# saveRDS(d1.emsumO,"models/study1/d1.emsumO.rds")
d1.emsumO <- readRDS("models/study1/d1.emsumO.rds")
# d1.empairO <- summary(pairs(emmeans(lmr1.0, ~ Outcome)))
# saveRDS(d1.empairO,"models/study1/d1.empairO.rds")
d1.empairO <- readRDS("models/study1/d1.empairO.rds")

## Descriptive means and se (used for plotting)
d1.sum <- d1 %>% 
...
# saveRDS(glmr1.i,"models/study1/glmr1i.rds")
glmr1.i <- readRDS("models/study1/glmr1i.rds")

## Main effect of agent (significant: p <.001,0.0004993)
# glmr1.2 <- glmer(Counterfactual ~ Outcome + (Agent*Outcome|Vignette) + (1|Subj), data=d1, family = "binomial",
#                  control = glmerControl(optimizer = "bobyqa"))
# glmr1.a <- anova(glmr1.1,glmr1.2)
# saveRDS(glmr1.a,"models/study1/glmr1a.rds")
glmr1.a <- readRDS("models/study1/glmr1a.rds")

# Main effect of outcome (significant: p =0.000646)
# glmr1.3 <- glmer(Counterfactual ~ Agent + (Agent*Outcome|Vignette) + (1|Subj), data=d1, family = "binomial",
#                  control = glmerControl(optimizer = "bobyqa"))
# glmr1.o <- anova(glmr1.1,glmr1.3)
# saveRDS(glmr1.o,"models/study1/glmr1o.rds")
glmr1.o <- readRDS("models/study1/glmr1o.rds")

### d1 counterfactual estimated marginal means & proportions

## Interaction effect:
# d1.cfsumAO <- summary(emmeans::emmeans(glmr1.0, ~ Agent*Outcome))
# saveRDS(d1.cfsumAO,"models/study1/d1.cfsumAO.rds")
d1.cfsumAO <- readRDS("models/study1/d1.cfsumAO.rds")
# d1.cfpairAO <- summary(pairs(emmeans::emmeans(glmr1.0, ~ Agent*Outcome)))
# saveRDS(d1.cfpairAO,"models/study1/d1.cfpairAO.rds")
d1.cfpairAO <- readRDS("models/study1/d1.cfpairAO.rds")

## Effect of Agent
# d1.cfsumA <- summary(emmeans::emmeans(glmr1.0, ~ Agent))
# saveRDS(d1.cfsumA,"models/study1/d1.cfsumA.rds")
d1.cfsumA <- readRDS("models/study1/d1.cfsumA.rds")
# d1.cfpairA <- summary(pairs(emmeans(glmr1.0, ~ Agent)))
# saveRDS(d1.cfpairA,"models/study1/d1.cfpairA.rds")
d1.cfpairA <- readRDS("models/study1/d1.cfpairA.rds")

## Effect of Outcome
# d1.cfsumO <- summary(emmeans::emmeans(glmr1.0, ~ Outcome))
# saveRDS(d1.cfsumO,"models/study1/d1.cfsumO.rds")
d1.cfsumO <- readRDS("models/study1/d1.cfsumO.rds")
# d1.cfpairO <- summary(pairs(emmeans(glmr1.0, ~ Outcome)))
# saveRDS(d1.cfpairO,"models/study1/d1.cfpairO.rds")
d1.cfpairO <- readRDS("models/study1/d1.cfpairO.rds")
...
## Mediation Analysis --> do cfs mediate the interaction effect between knowledge and outcome on causal judgments 
## (excluding cases of inanimate objects in the analysis below)

# create mediation dataframe
d1.m <- d1 %>% filter(Agent!="Object") %>%
        mutate(
          Counterfactual = case_when(
            Counterfactual=="Distal" ~ TRUE,
            Counterfactual=="Proximal" ~ FALSE),
          Agent = factor(Agent, levels = c("Ignorance","Knowledge"), labels = c(0, 1)),
          Outcome = factor(Outcome, levels = c("Bad","Good"), labels = c(0, 1)),
          # Recoding interaction term explicitly
          Interaction = case_when(
            Agent == 1 & Outcome == 0 ~ 0, ## Knowledge + Bad
            Agent == 1 & Outcome == 1 ~ 1, ## Knowledge + Good
            Agent == 0 & Outcome == 0 ~ 1, ## Ignorance + Bad
            Agent == 0 & Outcome == 1 ~ 0 ## Ignorance + Good
          )
          )

d1med.fit <- glmer(Counterfactual ~ Agent + Outcome + Interaction + (1|Subj), family="binomial", data=d1.m)
d1out.fit <- lmer(Cause ~ Counterfactual + Agent + Outcome + Interaction + (1|Subj), data=d1.m)
d1med.out <- mediate(d1med.fit, d1out.fit, sims=1000, treat="Interaction", mediator="Counterfactual")
...
d1.plot3 <- ggplot(d1.sumCorGraph, aes(y=CauseM, x=CntrfMr,color=Outcome))+
  ...
  scale_color_manual(values=c("#bdd7e7", "#2171b5")) +
  theme_bw() +
  theme(
    plot.background = element_blank()
    ,panel.grid.major = element_blank()
...
d2ademo <- read.csv("../data/d2a_demo.csv")

d2ademo$gender <- factor(c("Male","Female")[d2ademo$gender])
d2ademo$ethn <- factor(c("Black/African American","Hispanic/Latino","Asian/Pacific Islander","Native American/American Indian",
                         "White/Caucasian","Other")[d2ademo$ethn])
...
d2a.education <- table(d2ademo$edu)
```

In Experiment 2a, `r length(d2ademo$age)` participants (*M*~age~=`r round(mean(d2ademo$age, na.rm=T),digits=2)`, *SD*~age~=`r round(sd(d2ademo$age, na.rm=T),digits=2)`; `r d2a.gender[[1]]` females) were recruited through Amazon Mechanical Turk (<http://www.mturk.com>).
...
d2a <- read.csv("../data/d2a_data.csv")
...
# lmr2a.0 <- lmer(cause ~ knowledge * norm + (knowledge*norm|vign) + (1|subj), data=d2a,
#                  control = lmerControl(optimizer = "bobyqa"))
# lmr2a.1 <- lmer(cause ~ knowledge + norm + (knowledge*norm|vign) + (1|subj), data=d2a, 
#                 control = lmerControl(optimizer = "bobyqa"))
...
# lmr2a.2 <- lmer(cause ~ norm + (knowledge*norm|vign) + (1|subj), data=d2a,
#                 control = lmerControl(optimizer = "bobyqa"))
# lmr2a.k <- anova(lmr2a.1,lmr2a.2)
# saveRDS(lmr2a.k,"models/lmr2ak.rds")
lmr2a.k <- readRDS("models/lmr2ak.rds")

# main effect of norm (insignificant)
# lmr2a.3 <- lmer(cause ~ knowledge + (knowledge*norm|vign) + (1|subj), data=d2a,
#                 control = lmerControl(optimizer = "bobyqa"))
...
We analyzed participants' causal judgments, which revealed a main effect of *Knowledge*, $\chi^2(1)$ = `r round(lmr2a.k$Chisq[2],digits=2)`, *p* $<$ .001, such that agents knowingly bringing about harm were seen as more causal (*M* = `r round(d2a.emsumK$emmean[2],digits=2)`, 95% CI = [`r round(d2a.emsumK$lower.CL[2],digits=2)`, `r round(d2a.emsumK$upper.CL[2],digits=2)`]) than those who did so ignorantly  (*M* = `r round(d2a.emsumK$emmean[1],digits=2)`, 95% CI = [`r round(d2a.emsumK$lower.CL[1],digits=2)`, `r round(d2a.emsumK$upper.CL[1],digits=2)`]) regardless of the victim, *t*(`r round(d2a.empairK$df[1], digits=2)`) = `r sprintf("%.2f", d2a.empairK$t.ratio[1])`, *p* < .001, because we observed neither a main effect of *Harm Type*, $\chi^2(1)$ = `r round(lmr2a.n$Chisq[2],digits=2)`, *p* = `r round(lmr2a.n$'Pr(>Chisq)'[2],digits=3)`, nor a *Knowledge* $\times$ *Harm Type* interaction effect, $\chi^2(1)$ = `r round(lmr2a.i$Chisq[2],digits=2)`, *p* = `r round(lmr2a.i$'Pr(>Chisq)'[2],digits=3)`. In other words, causal judgments did not differ much between harm to self (*M* = `r round(d2a.emsumN$emmean[2],digits=2)`, 95% CI = [`r round(d2a.emsumN$lower.CL[2],digits=2)`, `r round(d2a.emsumN$upper.CL[2],digits=2)`]) and harm to others (*M* = `r round(d2a.emsumN$emmean[1],digits=2)`, 95% CI = [`r round(d2a.emsumN$lower.CL[1],digits=2)`, `r round(d2a.emsumN$upper.CL[1],digits=2)`]), *t*(`r sprintf("%.2f", d2a.empairN$df[1])`) = `r round(d2a.empairN$t.ratio[1], digits=2)`, *p* = `r sprintf("%.3f", d2a.empairN$p.value[1])` when the secondary outcome was negative. Taken together, findings suggest that other-harm and self-harm do not differentiate in regard to impact on causal judgments (*Fig*. 5).
...
d2a.fig <- ggplot(data=d2a.plot, aes(x=knowledge, y=causeMean, fill=knowledge)) +
  ...
  facet_grid(~norm, labeller = labeller(norm = c("Moral Norm" = "Other-Harm", "Rational Norm" = "Self-Harm"))) +
  ylab("Causation rating") +
  xlab("") +
  theme_bw() +
  theme(
...
d2bdemo <- read.csv("../data/d2b_demo.csv")

d2bdemo$gender <- factor(c("Male","Female")[d2bdemo$gender])
d2bdemo$ethn <- factor(c("Black/African American","Hispanic/Latino","Asian/Pacific Islander","Native American/American Indian",
                         "White/Caucasian","Other")[d2bdemo$ethn])
...
In Experiment 2b, `r length(d2bdemo$age)` participants (*M*~age~=`r round(mean(d2bdemo$age, na.rm=T),digits=2)`, *SD*~age~=`r round(sd(d2bdemo$age, na.rm=T),digits=2)`; `r d2b.gender[[1]]` females) were recruited through Amazon Mechanical Turk (<http://www.mturk.com>).
...
d2b <- read.csv("../data/d2b_data.csv")
...
d2cdemo <- read.csv("../data/d2c_demo.csv") 
...
In Experiment 2c, `r length(d2cdemo$age)` participants (*M*~age~=`r round(mean(d2cdemo$age, na.rm=T),digits=2)`, *SD*~age~=`r round(sd(d2cdemo$age, na.rm=T),digits=2)`; `r d2c.gender[[1]]` females) were recruited through Amazon Mechanical Turk (<http://www.mturk.com>).
...
d2c <- read.csv("../data/d2c_data.csv")

d2c$knowledge <- factor(d2c$knowledge, levels=c("Ignorance","Knowledge"))
d2c$norm <- factor(d2c$norm)
d2c$outcome <- factor(d2c$outcome, levels=c("Bad", "Good"))
d2c <- d2c %>%
  mutate(norm = case_when(norm =="Rationality" ~ "Rational Norm",
                          norm == "Morality" ~ "Moral Norm")) %>%
  mutate(outcome = case_when(outcome == "Bad" ~ "Bad Outcome",
                             outcome == "Good" ~ "Good Outcome"))


## 2c Main effects (knowledge significant <.001 0.0002386)
# lmr2c.main <- lmer(cause ~ knowledge + outcome + norm  + (knowledge*outcome|vign) + (1|subj), data=d2c,
#                    control = lmerControl(optimizer = "bobyqa"))
# 
# lmr2c.outcome <- lmer(cause ~ knowledge + norm + (knowledge*outcome|vign) + (1|subj),data=d2c,
#                       control = lmerControl(optimizer = "bobyqa"))
...
# lmr2c.knowledge <- lmer(cause ~ outcome + norm + (knowledge*outcome|vign) + (1|subj), data=d2c,
#                         control = lmerControl(optimizer = "bobyqa"))
...
# lmr2c.norm <- lmer(cause ~ outcome + knowledge + (knowledge*outcome|vign) + (1|subj), data=d2c,
#                    control = lmerControl(optimizer = "bobyqa"))
...
### 3-way interactions
# lmr2c.0 <- lmer(cause ~ knowledge * norm * outcome + (knowledge*outcome|vign) + (1|subj), data=d2c,
#                 control = lmerControl(optimizer = "bobyqa"))
# lmr2c.1 <- lmer(cause ~ (knowledge * norm) + (outcome * knowledge) + (outcome * norm) + (knowledge*outcome|vign) + (1|subj), data=d2c,
#                 control = lmerControl(optimizer = "bobyqa"))
...
# lmr2c.NK <- lmer(cause ~ (knowledge * outcome) + (outcome * norm) + (knowledge*outcome|vign) + (1|subj), data=d2c,
#                 control = lmerControl(optimizer = "bobyqa"))
...
# lmr2c.NO <- lmer(cause ~ (knowledge * outcome) + (knowledge * norm) + (knowledge*outcome|vign) + (1|subj), data=d2c,
#                 control = lmerControl(optimizer = "bobyqa"))
...
# lmr2c.OK <- lmer(cause ~ (knowledge * norm) + (outcome * norm) + (knowledge*outcome|vign) + (1|subj), data=d2c,
#                 control = lmerControl(optimizer = "bobyqa"))
...
# d2c.empairK <- summary(pairs(emmeans::emmeans(lmr2c.0, ~ knowledge, pbkrtest.limit = 3419)))
...
d2c.fig <- ggplot(data=d2c.plot, aes(x=knowledge, y=causeMean, fill=knowledge)) +
  ...
  facet_grid(outcome~norm, labeller = labeller(norm = c("Moral Norm" = "Other-Harm", "Rational Norm" = "Self-Harm"))) +
  ylab("Causation rating") +
  xlab("") +
  theme_bw() +
  theme(
...
## Rationality main effects (outcome significant <.001, knowledge <.001)
...
We then looked into interaction effects for moral ratings. A significant *Harm Type* $\times$ *Knowledge* interaction $\chi^2(1)$ = `r round(lmr2d.NxK.m$Chisq[2],digits=2)`, *p* <.001, was observed. More specifically, when agents acted ignorantly, moral judgment on them harming selves (*M* = `r round(d2dmor.emsumNK$emmean[3],digits=2)`, 95% CI = [`r round(d2dmor.emsumNK$lower.CL[3],digits=2)`, `r round(d2dmor.emsumNK$upper.CL[3],digits=2)`) and that on harming others (*M* = `r round(d2dmor.emsumNK$emmean[1],digits=2)`, 95% CI = [`r round(d2dmor.emsumNK$lower.CL[1],digits=2)`, `r round(d2dmor.emsumNK$upper.CL[1],digits=2)`) was not outstanding, but when acting knowingly, agents who harmed others were deemed much more immoral (*M* = `r round(d2dmor.emsumNK$emmean[2],digits=2)`, 95% CI = [`r round(d2dmor.emsumNK$lower.CL[2],digits=2)`, `r round(d2dmor.emsumNK$upper.CL[2],digits=2)`) than those who merely acted to their own disadvantage (*M* = `r round(d2dmor.emsumNK$emmean[4],digits=2)`, 95% CI = [`r round(d2dmor.emsumNK$lower.CL[4],digits=2)`, `r round(d2dmor.emsumNK$upper.CL[4],digits=2)`). There was also a (unexpectedly??) significant *Harm Type* $\times$ *Outcome* interaction, $\chi^2(1)$ = `r round(lmr2d.NxO.m$Chisq[2],digits=3)`, *p* $<$.001, such that when the final outcome was good (*t*(`r round(d2dmor.empairNO$df[5])`) = `r round(d2dmor.empairNO$t.ratio[5], digits=2)`, *p* = `r round(d2dmor.empairNO$p.value[5], digits=3)`), participants believed agents who committed harm on others were less moral (*M* = `r round(d2dmor.emsumNO$emmean[2],digits=2)`, 95% CI = [`r round(d2dmor.emsumNO$lower.CL[2],digits=2)`, `r round(d2dmor.emsumNO$upper.CL[2],digits=2)`]) than those who harmed only themselves (*M* = `r round(d2dmor.emsumNO$emmean[4],digits=2)`, 95% CI = [`r round(d2dmor.emsumNO$lower.CL[4],digits=2)`, `r round(d2dmor.emsumNO$upper.CL[4],digits=2)`), but when this secondary outcome was bad (*t*(`r round(d2dmor.empairNO$df[2])`) = `r round(d2dmor.empairNO$t.ratio[2], digits=2)`, *p* = `r round(d2dmor.empairNO$p.value[2], digits=3)`), this difference in morality rating between other-harming agents (*M* = `r round(d2dmor.emsumNO$emmean[1],digits=2)`, 95% CI = [`r round(d2dmor.emsumNO$lower.CL[1],digits=2)`, `r round(d2dmor.emsumNO$upper.CL[1],digits=2)`) and self-harming agents became smaller (*M* = `r round(d2dmor.emsumNO$emmean[3],digits=2)`, 95% CI = [`r round(d2dmor.emsumNO$lower.CL[3],digits=2)`, `r round(d2dmor.emsumNO$upper.CL[3],digits=2)`). Analyses did not yield statistically significant results in the *Knowledge* $\times$ *Outcome* interaction, $\chi^2(1)$ = `r round(lmr2d.OxK.m$Chisq[2],digits=2)`, *p* = `r round(lmr2d.OxK.m$'Pr(>Chisq)'[2], digits=3)` or the three-way *Knowledge* $\times$ *Harm Type* $\times$ *Outcome* interaction, $\chi^2(1)$ = `r round(lmr2d.i3x.m$Chisq[2],digits=2)`, *p* = `r round(lmr2d.i3x.m$'Pr(>Chisq)'[2],digits=3)`.
...
## 2d counterfactual main effects (norm significant 0.015, knowledge <.001)
...
```{r 2d CombinedAnalyses data, echo=FALSE, warning=FALSE, message=FALSE}
...
### attach ratings together
d2.r <- left_join(d2.sums,d2da.sums[d2da.sums$quest=="Morality",-5],by=c("knowledge","norm","outcome","vign")) %>%
  rename(mean.mor = mean,
         sd.mor = sd,
         se.mor = se,
         N.mor = N)

d2.r <- left_join(d2.r,d2da.sums[d2da.sums$quest=="Rationality",-5],by=c("knowledge","norm","outcome","vign")) %>%
  rename(mean.rat = mean,
         sd.rat = sd,
         se.rat = se,
         N.rat = N)

### load counterfactual choices and summarize
d2dc <- read.csv("../data/d2dc_cf_data.csv")
d2dc$cf.re <- factor(d2dc$cf.re,levels=c("External environment", "Agent"))
d2dc$norm <- factor(d2dc$norm)
d2dc$norm <- factor(c("Moral Norm","Rational Norm")[d2dc$norm])
d2dc$outcome <- factor(d2dc$outcome)
d2dc$outcome <- factor(c("Bad Outcome","Good Outcome")[d2dc$outcome])
...
### join everything together (i.e., summarized combined ratings, summarized morality+rationality ratings and summarized counterfactual choices)
d2_all <- left_join(d2.r,d2dc.sums,by=c("knowledge","norm","outcome","vign"))

          # d2_all$cf.meanS <- scale(d2_all$cf.mean)
          # d2_all$cause.meanS <- scale(d2_all$cause.mean) 

d2d_all_sum <- d2_all %>% group_by(knowledge,norm,outcome) %>%
  summarise(moral.Mean = mean(mean.mor),
            rational.Mean = mean(mean.rat),
            cause.Mean = mean(mean.cause),
            cf.Mean = mean(cf.mean))

```

```{r 2d combined causation analyses (2a-2c), echo=FALSE, warning=FALSE, message=FALSE}

### Causation combined (2a-2c) analyses

## 2d combined causation main effects (knowledge significant <.001 )
# lmr2abc.main <- lmer(cause ~ knowledge + outcome + norm  + (knowledge*outcome|vign) + (1|subj), data=d2)
# 
# lmr2abc.outcome <- lmer(cause ~ knowledge + norm + (knowledge*outcome|vign) + (1|subj),data=d2)
# lmr2abc.outcome <- anova(lmr2abc.main,lmr2abc.outcome)
# saveRDS(lmr2abc.outcome,"models/lmr2abc.outcome.rds")
lmr2abc.outcome <- readRDS("models/lmr2abc.outcome.rds")

# lmr2abc.knowledge <- lmer(cause ~ outcome + norm + (knowledge*outcome|vign) + (1|subj), data=d2)
# lmr2abc.knowledge <- anova(lmr2abc.main,lmr2abc.knowledge)
# saveRDS(lmr2abc.knowledge,"models/lmr2abc.knowledge.rds")
lmr2abc.knowledge <- readRDS("models/lmr2abc.knowledge.rds")

# lmr2abc.norm <- lmer(cause ~ outcome + knowledge + (knowledge*outcome|vign) + (1|subj), data=d2)
# lmr2abc.norm <- anova(lmr2abc.main,lmr2abc.norm)
# saveRDS(lmr2abc.norm,"models/lmr2abc.norm.rds")
lmr2abc.norm <- readRDS("models/lmr2abc.norm.rds")


## 2d combined causation interactions (only Outcome x Knowledge significant <.001)
### 3-way interactions
# lmr2abc.0 <- lmer(cause ~ knowledge * norm * outcome + (knowledge*outcome|vign) + (1|subj), data=d2,
#                   control = lmerControl(optimizer = "bobyqa"))
# lmr2abc.1 <- lmer(cause ~ (knowledge * norm) + (outcome * knowledge) + (outcome * norm) + (knowledge*outcome|vign) + (1|subj), data=d2,
#                   control = lmerControl(optimizer = "bobyqa"))
# lmr2abc.i3x <- anova(lmr2abc.0,lmr2abc.1)
# saveRDS(lmr2abc.i3x,"models/lmr2abci3x.rds")
lmr2abc.i3x <- readRDS("models/lmr2abci3x.rds")

### Norm x Knowledge interaction (insignificant)
# lmr2abc.NK <- lmer(cause ~ (knowledge * outcome) + (outcome * norm) + (knowledge*outcome|vign) + (1|subj), data=d2,
#                    control = lmerControl(optimizer = "bobyqa"))
# lmr2abc.NxK <- anova(lmr2abc.NK,lmr2abc.1)
# saveRDS(lmr2abc.NxK,"models/lmr2abcNxK.rds")
lmr2abc.NxK <- readRDS("models/lmr2abcNxK.rds")

### Norm x Outcome interaction (insignificant .06)
# lmr2abc.NO <- lmer(cause ~ (knowledge * outcome) + (knowledge * norm) + (knowledge*outcome|vign) + (1|subj), data=d2,
#                    control = lmerControl(optimizer = "bobyqa"))
# lmr2abc.NxO <- anova(lmr2abc.NO,lmr2abc.1)
# saveRDS(lmr2abc.NxO,"models/lmr2abcNxO.rds")
lmr2abc.NxO <- readRDS("models/lmr2abcNxO.rds")

### Outcome x Knowledge interaction (significant <.001)
# lmr2abc.OK <- lmer(cause ~ (knowledge * norm) + (outcome * norm) + (knowledge*outcome|vign) + (1|subj), data=d2,
#                    control = lmerControl(optimizer = "bobyqa"))
# lmr2abc.OxK <- anova(lmr2abc.OK,lmr2abc.1)
# saveRDS(lmr2abc.OxK,"models/lmr2abcOxK.rds")
lmr2abc.OxK <- readRDS("models/lmr2abcOxK.rds")


### 2d combined causation estimated marginal means
# d2abc.emsumK <- summary(emmeans::emmeans(lmr2abc.0, ~ knowledge, pbkrtest.limit = 6529))
# saveRDS(d2abc.emsumK,"models/d2abc.emsumK.rds")
d2abc.emsumK<- readRDS("models/d2abc.emsumK.rds")
# d2abc.empairK <- summary(pairs(emmeans::emmeans(lmr2abc.0, ~ knowledge, pbkrtest.limit = 6529)))
# saveRDS(d2abc.empairK,"models/d2abc.empairK.rds")
d2abc.empairK <- readRDS("models/d2abc.empairK.rds")

# d2abc.emsumOK <- summary(emmeans::emmeans(lmr2abc.0, ~ outcome*knowledge, pbkrtest.limit = 6529))
# saveRDS(d2abc.emsumOK,"models/d2abc.emsumOK.rds")
d2abc.emsumOK <- readRDS("models/d2abc.emsumOK.rds")
# d2abc.empairOK <- summary(emmeans::emmeans(lmr2abc.0, pairwise ~ outcome*knowledge, pbkrtest.limit = 6529))$contrasts
# saveRDS(d2abc.empairOK,"models/d2abc.empairOK.rds")
d2abc.empairOK <- readRDS("models/d2abc.empairOK.rds")
```
...
Combining and analyzing causal judgment data from Experiments 2a-2c, we discovered a significant main effect in in *Knowledge*, $\chi^2(1)$ = `r sprintf("%.2f", lmr2abc.knowledge$Chisq[2])`, *p* <.001 as well as a significant interaction effect between *Knowledge* $\times$ *Outcome*, $\chi^2(1)$ = `r round(lmr2abc.OxK$Chisq[2],digits=2)`, *p* <.001. In other words, knowledgeable agents were considered more causal (*M* = `r sprintf("%.2f", d2abc.emsumK$emmean[2])`, 95% CI = [`r round(d2abc.emsumK$lower.CL[2],digits=2)`, `r round(d2abc.emsumK$upper.CL[2],digits=2)`]) than ignorant agents (*M* = `r round(d2abc.emsumK$emmean[1],digits=2)`, 95% CI = [`r round(d2abc.emsumK$lower.CL[1],digits=2)`, `r round(d2abc.emsumK$upper.CL[1],digits=2)`]) for the secondary outcome, *t*(`r round(d2abc.empairK$df[1], digits=2)`) = `r round(d2abc.empairK$t.ratio[1], digits=2)`, *p* < .001, and the magnitude of this knowledge effect depends on the outcome valence, such that in negative secondary outcomes, more causation was assigned to knowledgeable agents (*M* = `r sprintf("%.2f", d2abc.emsumOK$emmean[3])`, 95% CI = [`r sprintf("%.2f", d2abc.emsumOK$lower.CL[3])`, `r sprintf("%.2f",d2abc.emsumOK$upper.CL[3])`]) than ignorant agents (*M* = `r round(d2abc.emsumOK$emmean[1],digits=2)`, 95% CI = [`r round(d2abc.emsumOK$lower.CL[1],digits=2)`, `r round(d2abc.emsumOK$upper.CL[1],digits=2)`]), *t*(`r round(d2abc.empairOK$df[2],2)`) = `r sprintf("%.2f", d2abc.empairOK$t.ratio[2])`, *p* <.001, while in positive outcomes, the difference in causal judgment became smaller between agents who acted knowingly (*M* = `r round(d2abc.emsumOK$emmean[4],digits=2)`, 95% CI = [`r round(d2abc.emsumOK$lower.CL[4],digits=2)`, `r sprintf("%.2f",d2abc.emsumOK$upper.CL[4])`]) and those who did so ignorantly (*M* = `r sprintf("%.2f", d2abc.emsumOK$emmean[2])`, 95% CI = [`r round(d2abc.emsumOK$lower.CL[2],digits=2)`, `r round(d2abc.emsumOK$upper.CL[2],digits=2)`]), *t*(`r round(d2abc.empairOK$df[5],2)`) = `r round(d2abc.empairOK$t.ratio[5], digits=2)`, *p* = `r round(d2abc.empairOK$p.value[5], digits=3)` (*Fig*. 9). These results align with our findings in Experiment 2c.
...
d3.cs_raw <- read.csv("../data/d3a_demo.csv")
d3.cs_demo <- d3.cs_raw[-(1:2),] %>%
  dplyr::select(c("age", "edu", "gender", "ethn", "ses", "ResponseId"))
...
For ratings of causation for outcomes that were categorized as probable versus improbable, `r length(unique(d3.cs_demo$ResponseId))` participants were recruited (*M*~age~=`r round(mean(as.numeric(d3.cs_demo$age), na.rm=T),digits=2)`, *SD*~age~=`r round(sd(as.numeric(d3.cs_demo$age), na.rm=T),digits=2)`; `r length(which(d3.cs_demo$gender=="Female"))` females), among which `r sum(d3.cs_demo$ses != "")` answered all our demographic questions.
...
d3.cspb <- read.csv("../data/d3a_data.csv") 
```
...
###cspb main effects (only knowledge significant 0.0001855)
# lmr3cspb.main <- lmer(causation.response ~ knowledge + outcome + norm  + (knowledge+outcome+norm|scenario) + (1|ResponseId), data=d3.cspb,
#                       control = lmerControl(optimizer = "bobyqa"))
# 
# lmr3cspb.outcome <- lmer(causation.response ~ knowledge + norm + (knowledge+outcome+norm|scenario) + (1|ResponseId), data=d3.cspb,
#                          control = lmerControl(optimizer = "bobyqa"))
...
# lmr3cspb.norm <- lmer(causation.response ~ outcome + knowledge + (knowledge+outcome+norm|scenario) + (1|ResponseId), data=d3.cspb,
#                       control = lmerControl(optimizer = "bobyqa"))
...
# lmr3cspb.knowledge <- lmer(causation.response ~ outcome + norm + (knowledge+outcome+norm|scenario) + (1|ResponseId), data=d3.cspb,
#                            control = lmerControl(optimizer = "bobyqa"))
...
## cspb interactions (no sig results)
# lmr3cspb.i0 <- lmer(causation.response ~ knowledge * norm * outcome + (knowledge+outcome+norm|scenario) + (1|ResponseId), data=d3.cspb,
#                     control = lmerControl(optimizer = "bobyqa"))
# lmr3cspb.i1 <- lmer(causation.response ~ (knowledge * norm) + (outcome * knowledge) + (outcome * norm)  + (knowledge+outcome+norm|scenario) + (1|ResponseId), data=d3.cspb,
#                     control = lmerControl(optimizer = "bobyqa"))
...
# lmr3cspb.NK <- lmer(causation.response ~ (knowledge * outcome) + (outcome * norm)  + (knowledge+outcome+norm|scenario) + (1|ResponseId), data=d3.cspb,
#                     control = lmerControl(optimizer = "bobyqa"))
...
# lmr3cspb.NO <- lmer(causation.response ~ (knowledge * outcome) + (knowledge * norm) + (knowledge+outcome+norm|scenario) + (1|ResponseId), data=d3.cspb,
#                     control = lmerControl(optimizer = "bobyqa"))
...
# lmr3cspb.OK <- lmer(causation.response ~ (knowledge * norm) + (outcome * norm)  + (knowledge+outcome+norm|scenario)  + (1|ResponseId), data=d3.cspb,
#                     control = lmerControl(optimizer = "bobyqa"))
...
# d3a.emsumK <- summary(emmeans::emmeans(lmr3cspb.i0, ~ knowledge, pbkrtest.limit = 3229))
# saveRDS(d3a.emsumK,"models/d3a.emsumK.rds")
d3a.emsumK <- readRDS("models/d3a.emsumK.rds")
# d3a.empairK <- summary(pairs(emmeans::emmeans(lmr3cspb.i0, ~ knowledge, pbkrtest.limit = 3229)))
...
We also probed interaction effects and did not find any interaction effects, including the three-way *Knowledge* $\times$ *Harm Type* $\times$ *Outcome* interaction, $\chi^2(1)$ = `r round(lmr3cspb.3way$Chisq[2],digits=2)`, *p* = `r max(.001,round(lmr3cspb.3way$'Pr(>Chisq)'[2],digits=3))`, and the two-way interactions of *Knowledge* $\times$ *Outcome*, $\chi^2(1)$ = `r round(lmr3cspb.OxK$Chisq[2],digits=2)`, *p* = `r round(lmr3cspb.OxK$'Pr(>Chisq)'[2],digits=3)`, *Knowledge* $\times$ *Harm Type*, $\chi^2(1)$ = `r round(lmr3cspb.NxK$Chisq[2],digits=2)`, *p* = `r sprintf("%.3f",lmr3cspb.NxK$'Pr(>Chisq)'[2])`, and *Outcome* $\times$ *Harm Type*, $\chi^2(1)$ = `r round(lmr3cspb.NxO$Chisq[2], digits=2)`, *p* = `r round(lmr3cspb.NxO$'Pr(>Chisq)'[2],digits=3)`.
...
d3cs.fig <- ggplot(data=d3cs, aes(x=knowledge, y=causationMean, fill=knowledge)) +
  ...
  facet_grid(outcome~norm, labeller = labeller(norm = c("Morality" = "Other-Harm", "Rationality" = "Self-Harm"),              
                                               outcome = c("Abnormal" = "Improbable Outcome", "Normal" = "Probable Outcome"))) +
  ylab("Causation rating") +
  xlab("") +
  theme_bw() +
  theme(
...
The results of Experiment 3a provide several key insights. First, we successfully replicated the knowledge effect observed in Experiment 1 and Experiments 2a–2c for a new set of downstream consequences. Specifically, agents who knowingly engaged in actions that brought harm to themselves or others were considered more causal for the unrelated and unforeseen secondary outcome. Second, we did not find an interaction effect between knowledge and the probability of the secondary outcome: //////




knowledge was more influential on causal judgments when the downstream secondary consequence was improbable than when it was probable. This interaction, in we observed in cases that vary descriptive normality, provides further evidence for a unified normality-account for causal judgments.

While this study provides good initial evidence for a more general interaction between outcome normality and agent knowledge, it remains possible that participants still inferred that that the improbable outcomes had a more negative valence than the probable outcomes. To more precisely investigate whether the interaction effect was driven by outcome valence or outcome normality, we next asked participants to make valence and normality judgments for probable and improbable outcomes in order to directly gauge how their perceptions interact in the context of descriptive normality.
...
d3.oc.pb_raw <- read.csv("../data/d3b_demo.csv")
d3.oc.pb_demo <- d3.oc.pb_raw[-(1:2),] %>%
  dplyr::select(c("age", "edu", "gender", "ethn", "ses", "ResponseId"))
...
For ratings of normality and valence in final outcomes that were probable or improbable, `r length(unique(d3.oc.pb_demo$ResponseId))` participants were recruited (*M*~age~=`r round(mean(as.numeric(d3.oc.pb_demo[d3.oc.pb_demo$age != 1984, ]$age), na.rm=T),digits=2)`, *SD*~age~=`r round(sd(as.numeric(d3.oc.pb_demo[d3.oc.pb_demo$age != 1984, ]$age), na.rm=T),digits=2)`; `r length(which(d3.oc.pb_demo$gender=="Female"))` females), among which `r sum(d3.oc.pb_demo$gender != "")+1` answered all our demographic questions.
All participants were recruited through Amazon Mechanical Turk (<http://www.mturk.com>).

```{r study3b dataset, echo=FALSE, warning=FALSE, message=FALSE}
d3.ocpb <- read.csv("../data/d3b_data.csv")
```
...
### Valence responses
#### ocpb valence main effects (norm significant 0.04725)
...
# lmr3ocpbval.norm <- lmer(responses ~ knowledge + outcome + (knowledge*outcome|scenario) + (1|ResponseId), data=d3.ocpb[d3.ocpb$question=="Valence",], 
#                          control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
...
# lmr3ocpbval.knowledge <- lmer(responses ~ outcome + norm + (knowledge*outcome|scenario) + (1|ResponseId), data=d3.ocpb[d3.ocpb$question=="Valence",], 
#                               control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
...
## ocpb valence two-way: Norm x Knowledge interaction (significant 0.01671)
# lmr3ocpbval.NK <- lmer(responses ~ (knowledge * outcome) + (outcome * norm) + (knowledge+outcome|scenario) + (1|ResponseId), data=d3.ocpb[d3.ocpb$question=="Valence",],
#                        control = lmerControl(optimizer = "bobyqa"))
...
# lmr3ocpbval.NO <- lmer(responses ~ (knowledge * outcome) + (knowledge * norm) + (knowledge+outcome|scenario) + (1|ResponseId), data=d3.ocpb[d3.ocpb$question=="Valence",], 
#                        control = lmerControl(optimizer = "bobyqa"))
...
## ocpb valence two-way: Outcome x Knowledge interaction (marginally significant, 0.05656)
# lmr3ocpbval.OK <- lmer(responses ~ (norm * outcome) + (knowledge * norm) + (knowledge+outcome|scenario) + (1|ResponseId), data=d3.ocpb[d3.ocpb$question=="Valence",],
#                        control = lmerControl(optimizer = "bobyqa"))
...
### Normality responses
#### ocpb normality main effects (outcome significant <.001, norm significant 0.002884)
...
## ocpb normality two-way: Norm x Outcome interaction (insignificant)
...
No participant was excluded from the analyses as long as the entire study was completed (*n* = `r length(unique(d3.ocpb$ResponseId))`).
...
We analyzed participants' valence ratings, which revealed a main effect of the type of *Harm Type*, $\chi^2(1)$ = `r sprintf("%.2f", lmr3ocpbval.norm$Chisq[2])`, *p* = `r round(lmr3ocpbval.norm$'Pr(>Chisq)'[2],digits=3)`, such that participants thought it was less good of a thing for the secondary outcome to have happened when the separate primary outcome caused harm to others (*M* = `r sprintf("%.2f", d3bval.emsumN$emmean[1])`, 95% CI = [`r round(d3bval.emsumN$lower.CL[1],digits=2)`, `r round(d3bval.emsumN$upper.CL[1],digits=2)`]) than when harm was inflicted on the agents themselves (*M* = `r round(d3bval.emsumN$emmean[2],digits=2)`, 95% CI = [`r round(d3bval.emsumN$lower.CL[2],digits=2)`, `r round(d3bval.emsumN$upper.CL[2],digits=2)`]), *t*(`r round(d3bval.empairN$df[1], 2)`) = `r round(d3bval.empairN$t.ratio[1], digits=2)`, *p* = `r round(d3bval.empairN$p.value[1], digits=3)`. We did not observe either a main effect of whether or not the agent had *Knowledge* about the outcome, $\chi^2(1)$ = `r round(lmr3ocpbval.knowledge$Chisq[2], digits=2)`, *p* = `r round(lmr3ocpbval.knowledge$'Pr(>Chisq)'[2],digits=3)`, or a main effect of the type of the *Outcome*, $\chi^2(1)$ = `r round(lmr3ocpbval.outcome$Chisq[2],digits=2)`, *p* = `r round(lmr3ocpbval.outcome$'Pr(>Chisq)'[2],digits=3)`. We also probed interaction effects and two stood out. The interaction of *Knowledge* $\times$ *Harm Type*, $\chi^2(1)$ = `r round(lmr3ocpbval.NxK$Chisq[2],digits=2)`, *p* = `r round(lmr3ocpbval.NxK$'Pr(>Chisq)'[2],digits=3)`, revealed that when the harmful primary consequence was related to others, more positive valence was attributed to knowledgeable agents (*M* = `r round(d3bval.emsumNK$emmean[2],digits=2)`, 95% CI = [`r round(d3bval.emsumNK$lower.CL[2],digits=2)`, `r round(d3bval.emsumNK$upper.CL[2],digits=2)`]) than the oblivious ones (*M* = `r round(d3bval.emsumNK$emmean[1],digits=2)`, 95% CI = [`r round(d3bval.emsumNK$lower.CL[1],digits=2)`, `r round(d3bval.emsumNK$upper.CL[1],digits=2)`]), *t*(`r round(d3bval.empairNK$df[1], 2)`) = `r sprintf("%.2f", d3bval.empairNK$t.ratio[1])`, *p* = `r round(d3bval.empairNK$p.value[1], digits=3)`; when the harm was related to the agents themselves, occurrences involving knowledgeable agents (*M* = `r round(d3bval.emsumNK$emmean[4],digits=2)`, 95% CI = [`r sprintf("%.2f",d3bval.emsumNK$lower.CL[4])`, `r round(d3bval.emsumNK$upper.CL[4],digits=2)`]) were considered less positive than those involving ignorant agents (*M* = `r round(d3bval.emsumNK$emmean[3],digits=2)`, 95% CI = [`r round(d3bval.emsumNK$lower.CL[3],digits=2)`, `r round(d3bval.emsumNK$upper.CL[3],digits=2)`]), *t*(`r round(d3bval.empairNK$df[6], 2)`) = `r round(d3bval.empairNK$t.ratio[6], digits=2)`, *p* = `r round(d3bval.empairNK$p.value[6], digits=3)`. *Knowledge* $\times$ *Outcome* interaction was marginally significant, $\chi^2(1)$ = `r round(lmr3ocpbval.OxK$Chisq[2],digits=2)`, *p* = `r round(lmr3ocpbval.OxK$'Pr(>Chisq)'[2],digits=3)`, suggesting that when secondary outcomes were probable, there was not a large difference in valence attribution between ignorant agents (*M* = `r round(d3bval.emsumOK$emmean[3],digits=2)`, 95% CI = [`r sprintf("%.2f",d3bval.emsumOK$lower.CL[3])`, `r round(d3bval.emsumOK$upper.CL[3],digits=2)`]) and knowledgeable agents (*M* = `r round(d3bval.emsumOK$emmean[4],digits=2)`, 95% CI = [`r round(d3bval.emsumOK$lower.CL[4],digits=2)`, `r sprintf("%.2f",d3bval.emsumOK$upper.CL[4])`]), *t*(`r round(d3bval.empairOK$df[6],2)`) = `r round(d3bval.empairOK$t.ratio[6], digits=2)`, *p* = `r round(d3bval.empairOK$p.value[6], digits=3)`, whereas in improbable secondary outcomes, those caused by ignorant agents (*M* = `r round(d3bval.emsumOK$emmean[1],digits=2)`, 95% CI = [`r sprintf("%.2f",d3bval.emsumOK$lower.CL[1])`, `r round(d3bval.emsumNK$upper.CL[1],2)`]) were perceived as better than those caused by knowledgeable agents (*M* = `r round(d3bval.emsumOK$emmean[2],digits=2)`, 95% CI = [`r round(d3bval.emsumOK$lower.CL[2],digits=2)`, `r round(d3bval.emsumNK$upper.CL[2],digits=2)`]), *t*(`r round(d3bval.empairOK$df[1],2)`) = `r round(d3bval.empairOK$t.ratio[1], digits=2)`, *p* = `r round(d3bval.empairOK$p.value[1], digits=3)`. We did not, however, see statistically significant effect in the three-way *Knowledge* $\times$ *Harm Type* $\times$ *Outcome* interaction, $\chi^2(1)$ = `r round(lmr3ocpbval.3way$Chisq[2],digits=2)`, *p* = `r round(lmr3ocpbval.3way$'Pr(>Chisq)'[2],digits=3)` or the *Outcome* $\times$ *Harm Type* interaction, $\chi^2(1)$ = `r round(lmr3ocpbval.NxO$Chisq[2],digits=2)`, *p* = `r round(lmr3ocpbval.NxO$'Pr(>Chisq)'[2],digits=3)`.

We then looked into participants' normality ratings and found main effects of both *Harm Type* and *Outcome*. For *Harm Type*, $\chi^2(1)$ = `r sprintf("%.2f", lmr3ocpbnor.norm$Chisq[2])`, *p* = `r round(lmr3ocpbnor.norm$'Pr(>Chisq)'[2],digits=3)`, participants believed the secondary outcome was less normal when the acting agent brought about harm to others  (*M* = `r round(d3bnor.emsumN$emmean[1],digits=2)`, 95% CI = [`r round(d3bnor.emsumN$lower.CL[1],digits=2)`, `r sprintf("%.2f", d3bnor.emsumN$upper.CL[1])`]) than to themselves (*M* = `r round(d3bnor.emsumN$emmean[2],digits=2)`, 95% CI = [`r sprintf("%.2f", d3bnor.emsumN$lower.CL[2])`, `r round(d3bnor.emsumN$upper.CL[2],digits=2)`]), *t*(`r round(d3bnor.empairN$df[1],2)`) = `r sprintf("%.2f",d3bnor.empairN$t.ratio[1])`, *p* = `r sprintf("%.3f",d3bnor.empairN$p.value[1])`. Unsurprisingly for *Outcome*, $\chi^2(1)$ = `r round(lmr3ocpbnor.outcome$Chisq[2],digits=2)`, *p* < .001, participants saw events that were unlikely to happen as less normal (*M* = `r round(d3bnor.emsumO$emmean[1],digits=2)`, 95% CI = [`r round(d3bnor.emsumO$lower.CL[1],digits=2)`, `r round(d3bnor.emsumO$upper.CL[1],digits=2)`]) than those that were likely to happen (*M* = `r round(d3bnor.emsumO$emmean[1],digits=2)`, 95% CI = [`r round(d3bnor.emsumO$lower.CL[1],digits=2)`, `r round(d3bnor.emsumO$upper.CL[1],digits=2)`]), *t*(`r round(d3bnor.empairO$df[1],2)`) = `r round(d3bnor.empairO$t.ratio[1], digits=2)`, *p* < .001. There was no main effect concerning *Knowledge* for normality ratings $\chi^2(1)$ = `r round(lmr3ocpbnor.knowledge$Chisq[2],digits=2)`, *p* = `r round(lmr3ocpbnor.knowledge$'Pr(>Chisq)'[2],digits=3)`. Analyses did not reveal any statistically significant interaction effects, including the three-way *Knowledge* $\times$ *Harm Type* $\times$ *Outcome* interaction, $\chi^2(1)$ = `r round(lmr3ocpbnor.3way$Chisq[2],digits=2)`, *p* = `r round(lmr3ocpbnor.3way$'Pr(>Chisq)'[2],digits=3)`, the *Harm Type* $\times$ *Outcome* interaction, $\chi^2(1)$ = `r round(lmr3ocpbnor.NxO$Chisq[2],digits=2)`, *p* = `r round(lmr3ocpbnor.NxO$'Pr(>Chisq)'[2],digits=3)`, the *Knowledge* $\times$ *Harm Type* interaction, $\chi^2(1)$ = `r  sprintf("%.2f",lmr3ocpbnor.NxK$Chisq[2])`, *p* = `r round(lmr3ocpbnor.NxK$'Pr(>Chisq)'[2],digits=3)`, and the *Outcome* $\times$ *Knowledge* interaction, $\chi^2(1)$ = `r  sprintf("%.2f",lmr3ocpbnor.OxK$Chisq[2])`, *p* = `r round(lmr3ocpbnor.OxK$'Pr(>Chisq)'[2],digits=3)`.
...
In the next experiment we follow this suggestion by asking whether the interaction effect in the case of bad vs. good outcomes may be able to be accounted for by differences in pereceived normality. Specifically, we return to manipulating outcome valence (rather than probability) and again ask participants to evaluate valence and normality for good and bad outcomes. If bad outcomes are indeed seen as less normal, we can infer that outcome valuence is influencing perceptions of outcome normality, and therefore move one step closer to a more parsimonious account of normality being impacted by both statistical probability and valence.
...
d3.oc.gb_raw <- read.csv("../data/d3c_demo.csv")
d3.oc.gb_demo <- d3.oc.gb_raw[-(1:2),] %>%
  dplyr::select(c("age", "edu", "gender", "ethn", "ses", "ResponseId"))
...
For ratings of normality and valence in final outcomes that were either good or bad, `r length(unique(d3.oc.gb_demo$ResponseId))` participants were recruited (*M*~age~=`r round(mean(as.numeric(d3.oc.gb_demo$age), na.rm=T),digits=2)`, *SD*~age~=`r round(sd(as.numeric(d3.oc.gb_demo$age), na.rm=T),digits=2)`; `r length(which(d3.oc.gb_demo$gender=="Female"))` females), among which `r sum(d3.oc.gb_demo$gender != "")` answered all our demographic questions.
All participants were recruited through Amazon Mechanical Turk (<http://www.mturk.com>).

```{r study3c dataset, echo=FALSE, warning=FALSE, message=FALSE}
d3.ocgb <- read.csv("../data/d3c_data.csv")
```

### Materials

In this experiment, participants completed 16 trials which each involved reading a vignette where an agent's action resulted in an outcome that was good or bad and the action would have harmed someone else or the agent themself. In addition, the agent was either ignorant or knowledgeable about the harm. As in Study 3a and Study 3b, the study used a 2 (Harm Type) $\times$ 2 (Agent Knowledge) $\times$ 2 (Outcome Valence) $\times$ 16 (Scenario) design that was administered in a mixed within- and between-subjects fashion, such that participants saw all 16 scenarios and on each trial were randomly assigned to read 1 of the 8 different versions of that scenario.
...
## ocgb valence two-way: Norm x Knowledge interaction (significant, 0.01349)
...
### Normality responses
#### ocgb normality main effects (outcome significant 0.003333, norm marginally significant 0.06314, knowledge significant 0.04289)
...
No participant was excluded from the analyses as long as the entire study was completed (*n* = `r length(unique(d3.ocgb$ResponseId))`). The primary analyses were conducted with linear mixed-effects models and estimated marginal means. More specifically, we included a random intercept for each participant and a random slope for the interaction between *Knowledge* and *Outcome* nested in each scenario. We did not include *Harm Type* in the random slope due to its minimal effect. Based on estimated marginal means analyses, we report both the estimated marginal means and the 95% confidence intervals.

## Results

Note that we report all analyses conducted for this experiment to ensure comprehensive coverage, including results that are not directly relevant to our primary predictions.

Analysis of participants' valence ratings revealed a main effect of *Harm Type*, $\chi^2(1)$ = `r round(lmr3ocgbval.norm$Chisq[2],digits=2)`, *p* <.001, where harm to others were seen as less good (*M* = `r round(d3cval.emsumN$emmean[1],digits=2)`, 95% CI = [`r round(d3cval.emsumN$lower.CL[1],digits=2)`, `r round(d3cval.emsumN$upper.CL[1],digits=2)`]) than harm to selves (*M* = `r round(d3cval.emsumN$emmean[2],digits=2)`, 95% CI = [`r round(d3cval.emsumN$lower.CL[2],digits=2)`, `r round(d3cval.emsumN$upper.CL[2],digits=2)`]), *t*(`r round(d3cval.empairN$df[1], digits=2)`) = `r sprintf("%.2f", d3cval.empairN$t.ratio[1])`, *p* < .001. And as expected, we found a main effect of *Outcome*, $\chi^2(1)$ = `r round(lmr3ocgbval.outcome$Chisq[2],digits=2)`, *p* <.001, such that participants agreed it was less good of a thing to have happened when the final outcome was bad (*M* = `r round(d3cval.emsumO$emmean[1],digits=2)`, 95% CI = [`r round(d3cval.emsumO$lower.CL[1],digits=2)`, `r sprintf("%.2f", d3cval.emsumO$upper.CL[1])`]) than when it was good (*M* = `r round(d3cval.emsumO$emmean[2],digits=2)`, 95% CI = [`r round(d3cval.emsumO$lower.CL[2],digits=2)`, `r round(d3cval.emsumO$upper.CL[2],digits=2)`]), *t*(`r round(d3cval.empairO$df[1], 2)`) = `r round(d3cval.empairO$t.ratio[1], digits=2)`, *p* < .001. Moreover, there exists an interaction between *Harm Type* and *Outcome*, $\chi^2(1)$ = `r round(lmr3ocgbval.NxO$Chisq[2],digits=2)`, *p* <.001, showing that in cases where final the outcome was bad, participants tended to think that harming others was less acceptable (*M* = `r round(d3cval.emsumNO$emmean[1],digits=2)`, 95% CI = [`r round(d3cval.emsumNO$lower.CL[1],digits=2)`, `r round(d3cval.emsumNO$upper.CL[1],digits=2)`]) than harming oneself (*M* = `r round(d3cval.emsumNO$emmean[2],digits=2)`, 95% CI = [`r round(d3cval.emsumNO$lower.CL[2],digits=2)`, `r sprintf("%.2f", d3cval.emsumNO$upper.CL[2])`]), *t*(`r round(d3cval.empairNO$df[1], 2)`) = `r round(d3cval.empairNO$t.ratio[1], digits=2)`, *p* = `r round(d3cval.empairNO$p.value[1], digits=3)`. When the outcome turned out to be good, this difference in valence judgment was even more salient between self-harm (*M* = `r round(d3cval.emsumNO$emmean[4],digits=2)`, 95% CI = [`r round(d3cval.emsumNO$lower.CL[4],digits=2)`, `r sprintf("%.2f",d3cval.emsumNO$upper.CL[4])`]) and other-harm (*M* = `r round(d3cval.emsumNO$emmean[3],digits=2)`, 95% CI = [`r round(d3cval.emsumNO$lower.CL[3],digits=2)`, *t*(`r round(d3cval.empairNO$df[6], 2)`) = `r round(d3cval.empairNO$t.ratio[6], digits=2)`, *p* < .001. In addition, the interaction between *Harm Type* and *Knowledge*, $\chi^2(1)$ = `r sprintf("%.2f",lmr3ocgbval.NxK$Chisq[2])`, *p* = `r round(lmr3ocgbval.NxK$'Pr(>Chisq)'[2],digits=3)`, reveals that when acting with knowledge, harming others was considered worse (*M* = `r round(d3cval.emsumNK$emmean[3],digits=2)`, 95% CI = [`r round(d3cval.emsumNK$lower.CL[3],digits=2)`, `r round(d3cval.emsumNK$upper.CL[3],digits=2)`]) than harming selves (*M* = `r round(d3cval.emsumNK$emmean[4],digits=2)`, 95% CI = [`r round(d3cval.emsumNK$lower.CL[4],digits=2)`, `r round(d3cval.emsumNK$upper.CL[4],digits=2)`], *t*(`r round(d3cval.empairNK$df[6], 2)`) = `r round(d3cval.empairNK$t.ratio[6], digits=2)`, *p* < .001. However, when agents acted obliviously, we saw less of a difference between participants' valence rating of other-harm (*M* = `r round(d3cval.emsumNK$emmean[1],digits=2)`, 95% CI = [`r round(d3cval.emsumNK$lower.CL[1],digits=2)`, `r round(d3cval.emsumNK$upper.CL[1],digits=2)`]) and that of self-harm (*M* = `r round(d3cval.emsumNK$emmean[2],digits=2)`, 95% CI = [`r round(d3cval.emsumNK$lower.CL[2],digits=2)`, `r round(d3cval.emsumNK$upper.CL[2],digits=2)`]), *t*(`r round(d3cval.empairNK$df[1], 2)`) = `r round(d3cval.empairNK$t.ratio[1], digits=2)`, *p* = `r round(d3cval.empairNK$p.value[1], digits=3)`. Testing the main effect of *Knowledge*, $\chi^2(1)$ = `r round(lmr3ocgbval.OxK$Chisq[2],digits=2)`, *p* = `r round(lmr3ocgbval.OxK$'Pr(>Chisq)'[2],digits=3)`, the interaction of *Outcome* $\times$ *Knowledge*, $\chi^2(1)$ = `r round(lmr3ocgbval.knowledge$Chisq[2],digits=2)`, *p* = `r round(lmr3ocgbval.knowledge$'Pr(>Chisq)'[2],digits=3)`, and the three-way interaction of *Knowledge* $\times$ *Harm Type* $\times$ *Outcome*, $\chi^2(1)$ = `r sprintf("%.2f", lmr3ocgbval.3way$Chisq[2])`, *p* = `r round(lmr3ocgbval.3way$'Pr(>Chisq)'[2],digits=3)`, did not yield statistically significant findings. 

When we looked into normality rating where we found evidence for all three main effects. The marginally significant *Harm Type* effect, $\chi^2(1)$ = `r round(lmr3ocgbnor.norm$Chisq[2],digits=2)`, *p* = `r round(lmr3ocgbnor.norm$'Pr(>Chisq)'[2],digits=3)`, suggests that participants considered harming others less normal (*M* = `r round(d3cnor.emsumN$emmean[1],digits=2)`, 95% CI = [`r round(d3cnor.emsumN$lower.CL[1],digits=2)`, `r round(d3cnor.emsumN$upper.CL[1],digits=2)`]) than harming selves (*M* = `r sprintf("%.2f", d3cnor.emsumN$emmean[2])`, 95% CI = [`r round(d3cnor.emsumN$lower.CL[2],digits=2)`, `r round(d3cnor.emsumN$upper.CL[2],digits=2)`]), *t*(`r round(d3cnor.empairN$df[1],2)`) = `r round(d3cnor.empairN$t.ratio[1], digits=2)`, *p* = `r round(d3cnor.empairN$p.value[1], digits=3)`. *Outcome* effect, $\chi^2(1)$ = `r round(lmr3ocgbnor.outcome$Chisq[2],digits=2)`, *p* = `r round(lmr3ocgbnor.outcome$'Pr(>Chisq)'[2],digits=3)`, indicates that good outcomes were seen as more normal (*M* = `r round(d3cnor.emsumO$emmean[2],digits=2)`, 95% CI = [`r round(d3cnor.emsumO$lower.CL[2],digits=2)`, `r sprintf("%.2f", d3cnor.emsumO$upper.CL[2])`]) than bad outcomes (*M* = `r round(d3cnor.emsumO$emmean[1],digits=2)`, 95% CI = [`r round(d3cnor.emsumO$lower.CL[1])`, `r round(d3cnor.emsumO$upper.CL[1],digits=2)`]), *t*(`r round(d3cnor.empairO$df[1], 2)`) = `r round(d3cnor.empairO$t.ratio[1], digits=2)`, *p* = `r round(d3cnor.empairO$p.value[1], digits=3)`. *Knowledge* effect, $\chi^2(1)$ = `r sprintf("%.2f", lmr3ocgbnor.knowledge$Chisq[2])`, *p* = `r round(lmr3ocgbnor.knowledge$'Pr(>Chisq)'[2],digits=3)`, reveals that participants thought it was more normal of a thing to have happened when the acting agent was knowledgeable  (*M* = `r round(d3cnor.emsumK$emmean[2],digits=2)`, 95% CI = [`r round(d3cnor.emsumK$lower.CL[2],digits=2)`, `r round(d3cnor.emsumK$upper.CL[2],digits=2)`]) than when the agent was ignorant (*M* = `r round(d3cnor.emsumK$emmean[1],digits=2)`, 95% CI = [`r round(d3cnor.emsumK$lower.CL[1],digits=2)`, `r round(d3cnor.emsumK$upper.CL[1],digits=2)`]), *t*(`r round(d3cnor.empairK$df[1],2)`) = `r round(d3cnor.empairK$t.ratio[1], digits=2)`, *p* = `r sprintf("%.3f", d3cnor.empairK$p.value[1])`. It is worth noting that we did not find any interaction effects, including the three-way interaction of *Knowledge* $\times$ *Harm Type* $\times$ *Outcome*, $\chi^2(1)$ = `r round(lmr3ocgbnor.3way$Chisq[2],digits=2)`, *p* = `r round(lmr3ocgbnor.3way$'Pr(>Chisq)'[2],digits=3)` and the two-way interactions of *Harm Type* $\times$ *Outcome*, $\chi^2(1)$ = `r round(lmr3ocgbnor.NxO$Chisq[2],digits=2)`, *p* = `r round(lmr3ocgbnor.NxO$'Pr(>Chisq)'[2],digits=3)`, *Harm Type* $\times$ *Knowledge*, $\chi^2(1)$ = `r round(lmr3ocgbnor.NxK$Chisq[2],digits=2)`, *p* = `r round(lmr3ocgbnor.NxK$'Pr(>Chisq)'[2],digits=3)`, *Knowledge* $\times$ *Outcome*, $\chi^2(1)$ = `r round(lmr3ocgbnor.OxK$Chisq[2],digits=2)`, *p* = `r round(lmr3ocgbnor.OxK$'Pr(>Chisq)'[2],digits=3)`.
...
d3ocgb.vale.fig <- ggplot(data=d3ocgb.vale, aes(x=knowledge, y=valenceMean, fill=knowledge)) +
  ...
  facet_grid(outcome~norm, labeller = labeller(norm = c("Morality" = "Other-Harm", "Rationality" = "Self-Harm"),              
                                               outcome = c("Bad" = "Bad Outcome", "Good" = "Good Outcome"))) +
  ylab("Valence rating") +
  xlab("") +
  theme_bw() +
  theme(
...
Across a range of scenarios, we have demonstrated the way in which participants' causal judgments are influenced by both an agent's knowledge of violating a norm, and by the normality of outcome about which participants are making causal judgments (e.g., good vs. bad outcomes). We have also systematically shown that such causal judgments are closely related to participants' counterfactual judgments (Gerstenberg et al., 2021; Lagnado et al., 2013; Lewis, 1986; Pearl, 2009). We next aim to model, more precisely, how these two factors, knowledge and outcome normality, affect causal judgments by impacting participants' counterfactual representations. 

Generally speaking, the relevant counterfactuals are roughly of the form, if the preceding event, $e$ hadn't happened, the outcome, $o$ wouldn't have occurred, or $\neg e \implies \neg o$. For causal reasoning, this kind of counterfactual conditional can be thought of as having two key pieces (see, e.g., Petrocelli et al., 2011). The first concerns the antecedent, specifically the accessibility of the possibility that the preceding event could have not occurred. The second concerns the conditional relationship between the antecedent and consequent, specifically, whether the outcome is dependent in some way or another on the preceding event. Intuitively, the core idea is people will perceive some prior event as causal of some outcome to the extent that (1) the possibility that the prior event could have not occurred is highly accessible, and (2) if that prior event had not occurred, the outcome would also not have occurred. Against this background, we want to model how knowledge and outcome normality are affecting these two different components of counterfactual representations.

Our running hypothesis is that changes to knowledge primarily affect the perceived accessibility of the antecedent in the counterfactual conditional, while changes to outcome normality primarily affect the perceived dependence relation between the antecedent and consequent. For knowledge: it seems intuitive enough that when an agent acts in way that that involves knowingly violating a norm, it is likely to increase the relevance of the possibility that the agent could have not done that action. For outcome normality: it seems plausible that abnormal outcomes are perceived as being more fragile, or dependent on the particular preceding events while more normal outcomes are perceived as more stable, and less dependent on the particular events that preceded them.

In Experiment 4, we empirically test this set of predictions by independently measuring participants' perceptions of (1) the accessibility of the antecedent in the counterfactual conditional and (2) the dependence relation between the antecedent and the consequent. We also combine these two judgments into a unified measure of counterfactual "potency" (Petrocelli et al., 2011; Gerstenberg, et al., 2018), which we use to predict prior participants causal judgments (from Experiment 2c). In Experiment 4a specifically, we used a 2 (Harm Type) $\times$ 2 (Agent Knowledge) $\times$ 2 (Outcome Valence) $\times$ 16 (Scenario) design that was administered in a mixed within- and between-subjects fashion, such that participants saw all 16 scenarios and on each trial were randomly assigned to read 1 of the 8 different versions of that scenario. 
...
```{r study4a participants, echo=FALSE, warning=FALSE, message=FALSE}

# GoodBad CounterFactual demo (age, gender, education, ethnicity, ses): 
d4.gb_raw <- read.csv("../data/d4a_demo.csv")
d4.gb_demo <- d4.gb_raw[-(1:2),] %>%
  dplyr::select(c("age", "edu", "gender", "ethn", "ses", "ResponseId"))
...
In Experiment 4a, `r length(unique(d4.gb_demo$ResponseId))` participants were recruited (*M*~age~=`r round(mean(as.numeric(d4.gb_demo$age), na.rm=T),digits=2)`, *SD*~age~=`r round(sd(as.numeric(d4.gb_demo$age), na.rm=T),digits=2)`; `r length(which(d4.gb_demo$gender=="Female"))` females), among which `r sum(d4.gb_demo$edu != "")` answered all our demographic questions. All participants were recruited through Amazon Mechanical Turk <http://www.mturk.com>.

```{r study4a dataset, echo=FALSE, warning=FALSE, message=FALSE}
d4gb <- read.csv("../data/d4a_data.csv", check.names = F) %>%
  mutate(`CF responses` = as.numeric(`CF responses`))
```

### Materials

Participants completed 16 trials which each involved reading a vignette where an agent's action resulted in an outcome that was good or bad (valence of occurrence). The action would have harmed either the agent her-/himself or someone else. In addition, the agent was either ignorant or knowledgeable about the norm violated.
...
After reading each vignette, participants answered two questions which target the two key components of the counterfactual conditional. We refer to the first question, which targets the accessibility of the possibility where the preceding event does not occur, as the "modal" question, and we refer to the second question, which targets the dependence between the antecedent and consequent, as the "counterfactual" question. To illustrate with our running example, these two questions would be:
...
```{r study4a md & cf interactions, echo=FALSE, warning=FALSE, message=FALSE}
## Md main effects (knowledge significant <.001, outcome significant 0.003137, norm insignificant; used knowledge+outcome for convergence)
# lmr4gb_md.main <- lmer(`CF responses` ~ knowledge + outcome + norm + (knowledge+outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Modal",],
#                        control = lmerControl(optimizer = "bobyqa"))
# 
# lmr4gb_md.outcome <- lmer(`CF responses` ~ knowledge + norm + (knowledge+outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Modal",],
#                           control = lmerControl(optimizer = "bobyqa"))
# lmr4gb_md.outcome <- anova(lmr4gb_md.main,lmr4gb_md.outcome)
# saveRDS(lmr4gb_md.outcome,"models/lmr4gb_md.outcome.rds")
lmr4gb_md.outcome <- readRDS("models/lmr4gb_md.outcome.rds")

# lmr4gb_md.norm <- lmer(`CF responses` ~ knowledge + outcome + (knowledge+outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Modal",],
#                        control = lmerControl(optimizer = "bobyqa"))
# lmr4gb_md.norm <- anova(lmr4gb_md.main,lmr4gb_md.norm)
# saveRDS(lmr4gb_md.norm,"models/lmr4gb_md.norm.rds")
lmr4gb_md.norm <- readRDS("models/lmr4gb_md.norm.rds")

# lmr4gb_md.knowledge <- lmer(`CF responses` ~ outcome + norm + (knowledge+outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Modal",],
#                             control = lmerControl(optimizer = "bobyqa"))
# lmr4gb_md.knowledge <- anova(lmr4gb_md.main,lmr4gb_md.knowledge)
# saveRDS(lmr4gb_md.knowledge,"models/lmr4gb_md.knowledge.rds")
lmr4gb_md.knowledge <- readRDS("models/lmr4gb_md.knowledge.rds")


## Md: Three-way interaction (insignificant)
# lm4gb_md.0 <- lmer(`CF responses` ~ outcome*knowledge*norm + (knowledge+outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Modal",],
#                    control = lmerControl(optimizer = "bobyqa"))
# lm4gb_md.1 <- lmer(`CF responses` ~ (knowledge*norm) + (outcome*knowledge) + (outcome*norm) + (knowledge+outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Modal",],
#                 control = lmerControl(optimizer = "bobyqa"))
# lm4gb_md.3way <- anova(lm4gb_md.0, lm4gb_md.1)
# saveRDS(lm4gb_md.3way, "models/lm4gb_md.3way.rds")
lm4gb_md.3way <- readRDS("models/lm4gb_md.3way.rds")

## Md: Outcome x Knowledge interaction (insignificant)
# lm4gb_md.2 <- lmer(`CF responses` ~ (knowledge*norm) + (outcome*norm) + (knowledge+outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Modal",],
#                 control = lmerControl(optimizer = "bobyqa"))
# lm4gb_md.OxK <- anova(lm4gb_md.1,lm4gb_md.2)
# saveRDS(lm4gb_md.OxK,"models/lm4gb_md.OxK.rds")
lm4gb_md.OxK <- readRDS("models/lm4gb_md.OxK.rds")

## Md: Outcome x Norm interaction (insignificant)
# lm4gb_md.3 <- lmer(`CF responses` ~ (knowledge*norm) + (outcome*knowledge) + (knowledge+outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Modal",],
#                 control = lmerControl(optimizer = "bobyqa"))
# lm4gb_md.OxN <- anova(lm4gb_md.1,lm4gb_md.3)
# saveRDS(lm4gb_md.OxN,"models/lm4gb_md.OxN.rds")
lm4gb_md.OxN <- readRDS("models/lm4gb_md.OxN.rds")

## Md: Norm x Knowledge interaction (insignificant)
# lm4gb_md.4 <- lmer(`CF responses` ~ (knowledge*outcome) + (outcome*norm) + (knowledge+outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Modal",],
#                 control = lmerControl(optimizer = "bobyqa"))
# lm4gb_md.NxK <- anova(lm4gb_md.1,lm4gb_md.4)
# saveRDS(lm4gb_md.NxK,"models/lm4gb_md.NxK.rds")
lm4gb_md.NxK <- readRDS("models/lm4gb_md.NxK.rds")

### Modality estimated marginal means
# d4gbmd.emsumO <- summary(emmeans::emmeans(lm4gb_md.0, ~ outcome, pbkrtest.limit = 3183))
# saveRDS(d4gbmd.emsumO,"models/d4gbmd.emsumO.rds")
d4gbmd.emsumO <- readRDS("models/d4gbmd.emsumO.rds")
# d4gbmd.empairO <- summary(pairs(emmeans(lm4gb_md.0, ~ outcome, pbkrtest.limit = 3183)))
# saveRDS(d4gbmd.empairO,"models/d4gbmd.empairO.rds")
d4gbmd.empairO <- readRDS("models/d4gbmd.empairO.rds")

# d4gbmd.emsumK <- summary(emmeans::emmeans(lm4gb_md.0, ~ knowledge, pbkrtest.limit = 3183))
# saveRDS(d4gbmd.emsumK,"models/d4gbmd.emsumK.rds")
d4gbmd.emsumK <- readRDS("models/d4gbmd.emsumK.rds")
# d4gbmd.empairK <- summary(pairs(emmeans(lm4gb_md.0, ~ knowledge, pbkrtest.limit = 3183)))
# saveRDS(d4gbmd.empairK,"models/d4gbmd.empairK.rds")
d4gbmd.empairK <- readRDS("models/d4gbmd.empairK.rds")



## Cf main effects (outcome significant <.001, knowledge and norm insignificant)
# lmr4gb_cf.main <- lmer(`CF responses` ~ knowledge + outcome + norm + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Counterfactual",],
#                        control = lmerControl(optimizer = "bobyqa"))

# lmr4gb_cf.outcome <- lmer(`CF responses` ~ knowledge + norm + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Counterfactual",],
#                        control = lmerControl(optimizer = "bobyqa"))
# lmr4gb_cf.outcome <- anova(lmr4gb_cf.main,lmr4gb_cf.outcome)
# saveRDS(lmr4gb_cf.outcome,"models/lmr4gb_cf.outcome.rds")
lmr4gb_cf.outcome <- readRDS("models/lmr4gb_cf.outcome.rds")

# lmr4gb_cf.norm <- lmer(`CF responses` ~ knowledge + outcome + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Counterfactual",],
#                     control = lmerControl(optimizer = "bobyqa"))
# lmr4gb_cf.norm <- anova(lmr4gb_cf.main,lmr4gb_cf.norm)
# saveRDS(lmr4gb_cf.norm,"models/lmr4gb_cf.norm.rds")
lmr4gb_cf.norm <- readRDS("models/lmr4gb_cf.norm.rds")

# lmr4gb_cf.knowledge <- lmer(`CF responses` ~ outcome + norm + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Counterfactual",],
#                          control = lmerControl(optimizer = "bobyqa"))
# lmr4gb_cf.knowledge <- anova(lmr4gb_cf.main,lmr4gb_cf.knowledge)
# saveRDS(lmr4gb_cf.knowledge,"models/lmr4gb_cf.knowledge.rds")
lmr4gb_cf.knowledge <- readRDS("models/lmr4gb_cf.knowledge.rds")


## Cf: Three-way interaction (insignificant)
# lm4gb_cf.0 <- lmer(`CF responses` ~ outcome*knowledge*norm + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Counterfactual",],
#                    control = lmerControl(optimizer = "bobyqa"))
# lm4gb_cf.1 <- lmer(`CF responses` ~ (knowledge*norm) + (outcome*knowledge) + (outcome*norm) + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Counterfactual",],
#                 control = lmerControl(optimizer = "bobyqa"))
# lm4gb_cf.3way <- anova(lm4gb_cf.0, lm4gb_cf.1)
# saveRDS(lm4gb_cf.3way, "models/lm4gb_cf.3way.rds")
lm4gb_cf.3way <- readRDS("models/lm4gb_cf.3way.rds")

## Cf: Outcome x Knowledge interaction (significant 0.01323)
# lm4gb_cf.2 <- lmer(`CF responses` ~ (knowledge*norm) + (outcome*norm) + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Counterfactual",],
#                 control = lmerControl(optimizer = "bobyqa"))
# lm4gb_cf.OxK <- anova(lm4gb_cf.1,lm4gb_cf.2)
# saveRDS(lm4gb_cf.OxK,"models/lm4gb_cf.OxK.rds")
lm4gb_cf.OxK <- readRDS("models/lm4gb_cf.OxK.rds")

## Cf: Outcome x Norm interaction (insignificant)
# lm4gb_cf.3 <- lmer(`CF responses` ~ (knowledge*norm) + (outcome*knowledge) + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Counterfactual",],
#                 control = lmerControl(optimizer = "bobyqa"))
# lm4gb_cf.OxN <- anova(lm4gb_cf.1,lm4gb_cf.3)
# saveRDS(lm4gb_cf.OxN,"models/lm4gb_cf.OxN.rds")
lm4gb_cf.OxN <- readRDS("models/lm4gb_cf.OxN.rds")

## Cf: Norm x Knowledge interaction (insignificant)
# lm4gb_cf.4 <- lmer(`CF responses` ~ (knowledge*outcome) + (outcome*norm) + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb[d4gb$question=="Counterfactual",],
#                 control = lmerControl(optimizer = "bobyqa"))
# lm4gb_cf.NxK <- anova(lm4gb_cf.1,lm4gb_cf.4)
# saveRDS(lm4gb_cf.NxK,"models/lm4gb_cf.NxK.rds")
lm4gb_cf.NxK <- readRDS("models/lm4gb_cf.NxK.rds")


### Counterfactural estimated marginal means
# d4gbcf.emsumO <- summary(emmeans::emmeans(lm4gb_cf.0, ~ outcome, pbkrtest.limit = 3213))
# saveRDS(d4gbcf.emsumO,"models/d4gbcf.emsumO.rds")
d4gbcf.emsumO <- readRDS("models/d4gbcf.emsumO.rds")
# d4gbcf.empairO <- summary(pairs(emmeans(lm4gb_cf.0, ~ outcome, pbkrtest.limit = 3213)))
# saveRDS(d4gbcf.empairO,"models/d4gbcf.empairO.rds")
d4gbcf.empairO <- readRDS("models/d4gbcf.empairO.rds")

# d4gbcf.emsumOK <- summary(emmeans::emmeans(lm4gb_cf.0, ~ outcome*knowledge, pbkrtest.limit = 3213))
# saveRDS(d4gbcf.emsumOK,"models/d4gbcf.emsumOK.rds")
d4gbcf.emsumOK <- readRDS("models/d4gbcf.emsumOK.rds")
# d4gbcf.empairOK <- summary(emmeans::emmeans(lm4gb_cf.0, pairwise ~ outcome*knowledge, pbkrtest.limit = 3213))$contrasts
# saveRDS(d4gbcf.empairOK,"models/d4gbcf.empairOK.rds")
d4gbcf.empairOK <- readRDS("models/d4gbcf.empairOK.rds")
...
We first explored participants' modal ratings and found a significant main effect of *Knowledge*, $\chi^2(1)$ = `r round(lmr4gb_md.knowledge$Chisq[2], digits=2)`, *p* <.001, such that participants thought it was less possible for ignorant agents to have not acted in the way they did (*M* = `r round(d4gbmd.emsumK$emmean[1],digits=2)`, 95% CI = [`r round(d4gbmd.emsumK$lower.CL[1], digits=2)`, `r round(d4gbmd.emsumK$upper.CL[1], digits=2)`]) than knowledgeable agents (*M* = `r round(d4gbmd.emsumK$emmean[2],digits=2)`, 95% CI = [`r round(d4gbmd.emsumK$lower.CL[2],digits=2)`, `r round(d4gbmd.emsumK$upper.CL[2],digits=2)`]), *t*(`r round(d4gbmd.empairK$df, digits=2)`) = `r round(d4gbmd.empairK$t.ratio[1], 2)`, *p* `r ifelse(d4gbmd.empairK$p.value < 0.001, "< .001", paste0("= ", round(d4gbmd.empairK$p.value, 3)))`. There was also a significant main effect of *Outcome*, $\chi^2(1)$ = `r round(lmr4gb_md.outcome$Chisq[2],2)`, *p* = `r round(lmr4gb_md.outcome$'Pr(>Chisq)'[2],digits=3)`. In other words, participants believed that it was more plausible for the agent to have acted differently when the secondary outcome turned out to be bad (*M* = `r round(d4gbmd.emsumO$emmean[1],digits=2)`, 95% CI = [`r round(d4gbmd.emsumO$lower.CL[1], digits=2)`, `r round(d4gbmd.emsumO$upper.CL[1], digits=2)`]) than when it turned out to be good (*M* = `r round(d4gbmd.emsumO$emmean[2],digits=2)`, 95% CI = [`r sprintf("%.2f", d4gbmd.emsumO$lower.CL[2])`, `r round(d4gbmd.emsumO$upper.CL[2],digits=2)`]), *t*(`r round(d4gbmd.empairO$df, digits=2)`) = `r round(d4gbmd.empairO$t.ratio[1], 2)`, *p* `r ifelse(d4gbmd.empairO$p.value < 0.001, "< .001", paste0("= ", round(d4gbmd.empairO$p.value, 3)))`. We did not observe an effect of *Harm Type*, $\chi^2(1)$ = `r round(lmr4gb_md.norm$Chisq[2],digits=2)`, *p* = `r round(lmr4gb_md.norm$'Pr(>Chisq)'[2], 3)`. Analyses of interaction effects in modality ratings did not yield any significant results, including the *Knowledge* $\times$ *Harm Type* $\times$ *Outcome* interaction, $\chi^2(1)$ = `r round(lm4gb_md.3way$Chisq[2],digits=2)`, *p* = `r round(lm4gb_md.3way$'Pr(>Chisq)'[2],digits=3)`,  *Knowledge* $\times$ *Outcome* interaction, $\chi^2(1)$ = `r round(lm4gb_md.OxK$Chisq[2],digits=2)`, *p* = `r round(lm4gb_md.OxK$'Pr(>Chisq)'[2], digits=3)`. We omitted interactions involving the *Harm Type* condition due to its minimal effect.

We then looked into counterfactual ratings, and discovered a significant main effect of *Outcome*, $\chi^2(1)$ = `r round(lmr4gb_cf.outcome$Chisq[2],digits=2)`, *p* `r ifelse(lmr4gb_cf.outcome$'Pr(>Chisq)' < 0.001, "< .001", paste0("= ", round(lmr4gb_cf.outcome$'Pr(>Chisq)', 3)))`, such that compared to good outcomes (*M* = `r round(d4gbcf.emsumO$emmean[2],digits=2)`, 95% CI = [`r round(d4gbcf.emsumO$lower.CL[2],2)`, `r round(d4gbcf.emsumO$upper.CL[2],digits=2)`]), participants were more inclined to think that in bad outcomes, things could have been different if the agents had acted differently (*M* = `r round(d4gbcf.emsumO$emmean[1],digits=2)`, 95% CI = [`r round(d4gbcf.emsumO$lower.CL[1],digits=2)`, `r round(d4gbcf.emsumO$upper.CL[1],digits=2)`]), *t*(`r round(d4gbcf.empairO$df, digits=2)`) = `r round(d4gbcf.empairO$t.ratio[1], 2)`, *p* `r ifelse(d4gbcf.empairO$p.value < 0.001, "< .001", paste0("= ", round(d4gbcf.empairO$p.value, 3)))`. Results did not reveal significant main effects in *Knowledge*, $\chi^2(1)$ = `r round(lmr4gb_cf.knowledge$Chisq[2],digits=2)`, *p* = `r round(lmr4gb_cf.knowledge$'Pr(>Chisq)'[2],digits=3)` or *Harm Type*, $\chi^2(1)$ = `r round(lmr4gb_cf.norm$Chisq[2],digits=2)`, *p* = `r round(lmr4gb_cf.norm$'Pr(>Chisq)'[2],digits=3)`. Analyses did, however, show a significant *Knowledge* $\times$ *Outcome* interaction effect in counterfactual ratings, $\chi^2(1)$ = `r round(lm4gb_cf.OxK$Chisq[2],digits=2)`, *p* = `r sprintf("%.3f",lm4gb_cf.OxK$'Pr(>Chisq)'[2])`, such that in good secondary outcomes, there was an almost negligible difference in participants' belief that the consequence could have been circumvented with an ignorant agent (*M* = `r round(d4gbcf.emsumOK$emmean[2],digits=2)`, 95% CI = [`r round(d4gbcf.emsumOK$lower.CL[2],digits=2)`, `r round(d4gbcf.emsumOK$upper.CL[2],digits=2)`]) as opposed to a knowledgeable agent (*M* = `r round(d4gbcf.emsumOK$emmean[4],digits=2)`, 95% CI = [`r sprintf("%.2f",d4gbcf.emsumOK$lower.CL[4])`, `r round(d4gbcf.emsumOK$upper.CL[4],digits=2)`]), *t*(`r round(d4gbcf.empairOK$df[5], digits=2)`) = `r round(d4gbcf.empairOK$t.ratio[5], 2)`, *p* `r ifelse(d4gbcf.empairOK$p.value[5] < 0.001, "< .001", paste0("= ", round(d4gbcf.empairOK$p.value[5], 3)))`. Whereas in bad outcomes, participants were more likely to think that when agents were knowledgeable (*M* = `r round(d4gbcf.emsumOK$emmean[3],digits=2)`, 95% CI = [`r round(d4gbcf.emsumOK$lower.CL[3],digits=2)`, `r round(d4gbcf.emsumOK$upper.CL[3],digits=2)`]), a different action could have resulted in a different outcome than when agents were ignorant (*M* = `r round(d4gbcf.emsumOK$emmean[1],digits=2)`, 95% CI = [`r round(d4gbcf.emsumOK$lower.CL[1],digits=2)`, `r round(d4gbcf.emsumOK$upper.CL[1],digits=2)`]), *t*(`r round(d4gbcf.empairOK$df[2], digits=2)`) = `r round(d4gbcf.empairOK$t.ratio[2], 2)`, *p* `r round(d4gbcf.empairOK$p.value[2], 3)`. Again, there was not a significant three-way *Knowledge* $\times$ *Harm Type* $\times$ *Outcome* interaction effect, $\chi^2(1)$ = `r round(lm4gb_cf.3way$Chisq[2],digits=2)`, *p* = `r sprintf("%.3f", lm4gb_cf.3way$'Pr(>Chisq)'[2])`, nor any interaction effects involving *Harm Type*.

```{r, study4a plots, fig.width=8.5, fig.height=7, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Participants' modality ratings (top) and counterfactuality ratings (bottom) regarding knowledgeable/ignorant agents who violated moral/rational norms when a separate outcome occurred that is normal/abnormal."}
...
d4gb.modality <- ggplot(data=d4gb.md, aes(x=knowledge, y=mdMean, fill=knowledge)) +
  geom_violin(alpha=.6, linewidth=.1) +
  scale_fill_manual(values=c("#bdd7e7", "#2171b5")) + ###bdd7e7 #6baed6
  geom_point(stat="identity",position=position_jitterdodge(jitter.width=.1,dodge.width=.9),alpha=.05, color="black") +
  geom_point(data=d4gb.md.sum, aes(x=knowledge, y=mdMean, fill=knowledge), shape=17, size=2,position=position_dodge(.9), color="black") +
  geom_line(data=d4gb.md.sum, aes(x=knowledge, y=mdMean, group=norm),linewidth=1) +
  geom_errorbar(data=d4gb.md.sum,aes(ymin=mdMean-mdsumSE, ymax=mdMean+mdsumSE), width=.2, position=position_dodge(.9),color="black") +
  facet_grid(outcome~norm, labeller = labeller(norm = c("Morality" = "Other-Harm", "Rationality" = "Self-Harm"), 
             outcome = c("Bad" = "Bad Outcome", "Good" = "Good Outcome"))) +
  ...
  theme(
    ...
    axis.ticks = element_blank()) +
  scale_y_continuous(breaks=c(1, 3, 5, 7))
#ggsave("d4gb.modality.pdf", width=8, height=12, path="figures")
d4gb.modality


### d4a counterfactual plot
d4gb.cf <- d4gb %>%
...
d4gb.counterfactual <- ggplot(data=d4gb.cf, aes(x=knowledge, y=cfMean, fill=knowledge)) +
  geom_violin(alpha=.6, linewidth=.1) +
  scale_fill_manual(values=c("#bdd7e7", "#2171b5")) + ###bdd7e7 #6baed6
  geom_point(stat="identity",position=position_jitterdodge(jitter.width=.1,dodge.width=.9),alpha=.05, color="black") +
  geom_point(data=d4gb.cf.sum, aes(x=knowledge, y=cfMean, fill=knowledge), shape=17, size=2,position=position_dodge(.9), color="black") +
  geom_line(data=d4gb.cf.sum, aes(x=knowledge, y=cfMean, group=norm),size=1) +
  geom_errorbar(data=d4gb.cf.sum,aes(ymin=cfMean-cfsumSE, ymax=cfMean+cfsumSE), width=.2, position=position_dodge(.9),color="black") +
  facet_grid(outcome~norm, labeller = labeller(norm = c("Morality" = "Other-Harm", "Rationality" = "Self-Harm"), 
             outcome = c("Bad" = "Bad Outcome", "Good" = "Good Outcome"))) +
  ...
  theme(
    ...
    axis.ticks = element_blank()) +
  scale_y_continuous(breaks=c(1, 3, 5, 7))
#ggsave("d4gb.counterfactual.pdf", width=8, height=12, path="figures")
d4gb.counterfactual

```

```{r study4a potency & study2c causation, echo=FALSE, warning=FALSE, message=FALSE}
## 4a potency: for each condition in a scenario, use causal to predict cf*md --> create a new column potency
d4gb_pot <- d4gb %>%
...
## main effects (outcome significant <.001, knowledge significant .002, norm insignificant)
# lmr4gbpot.main <- lmer(Potency ~ knowledge + outcome + norm + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb_pot,
#                        control = lmerControl(optimizer = "bobyqa"))

# lmr4gbpot.outcome <- lmer(Potency ~ knowledge + norm + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb_pot,
#                           control = lmerControl(optimizer = "bobyqa"))
# lmr4gbpot.outcome <- anova(lmr4gbpot.main,lmr4gbpot.outcome)
# saveRDS(lmr4gbpot.outcome,"models/lmr4gbpot.outcome.rds")
lmr4gbpot.outcome <- readRDS("models/lmr4gbpot.outcome.rds")

# lmr4gbpot.norm <- lmer(Potency ~ knowledge + outcome + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb_pot,
#                        control = lmerControl(optimizer = "bobyqa"))
# lmr4gbpot.norm <- anova(lmr4gbpot.main,lmr4gbpot.norm)
# saveRDS(lmr4gbpot.norm,"models/lmr4gbpot.norm.rds")
lmr4gbpot.norm <- readRDS("models/lmr4gbpot.norm.rds")

# lmr4gbpot.knowledge <- lmer(Potency ~ outcome + norm + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb_pot,
#                             control = lmerControl(optimizer = "bobyqa"))
# lmr4gbpot.knowledge <- anova(lmr4gbpot.main,lmr4gbpot.knowledge)
# saveRDS(lmr4gbpot.knowledge,"models/lmr4gbpot.knowledge.rds")
lmr4gbpot.knowledge <- readRDS("models/lmr4gbpot.knowledge.rds")

## three-way interaction (marginally insignificant .05414)
# lmr4gbpot.i0 <- lmer(Potency ~ knowledge * norm * outcome + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb_pot)
# lmr4gbpot.i1 <- lmer(Potency ~ (knowledge * norm) + (outcome * knowledge) + (outcome * norm) + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb_pot)
# lmr4gbpot.3way <- anova(lmr4gbpot.i0,lmr4gbpot.i1)
# saveRDS(lmr4gbpot.3way,"models/lmr4gbpot.3way.rds")
lmr4gbpot.3way <- readRDS("models/lmr4gbpot.3way.rds")

## two-way: Norm x Knowledge interaction (insignificant)
# lmr4gbpot.NK <- lmer(Potency ~ (knowledge * outcome) + (outcome * norm) + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb_pot)
# lmr4gbpot.NxK <- anova(lmr4gbpot.i1, lmr4gbpot.NK)
# saveRDS(lmr4gbpot.NxK,"models/lmr4gbpot.NxK.rds")
lmr4gbpot.NxK <- readRDS("models/lmr4gbpot.NxK.rds")

## two-way: Norm x Outcome interaction (insignificant)
# lmr4gbpot.NO <- lmer(Potency ~ (knowledge * outcome) + (knowledge * norm) + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb_pot)
# lmr4gbpot.NxO <- anova(lmr4gbpot.i1, lmr4gbpot.NO)
# saveRDS(lmr4gbpot.NxO,"models/lmr4gbpot.NxO.rds")
lmr4gbpot.NxO <- readRDS("models/lmr4gbpot.NxO.rds")

# two-way: Outcome x Knowledge interaction (significant, 0.01897)
# lmr4gbpot.OK <- lmer(Potency ~ (norm * outcome) + (knowledge * norm) + (knowledge*outcome|scenario) + (1|ResponseId), data=d4gb_pot)
# lmr4gbpot.OxK <- anova(lmr4gbpot.i1,lmr4gbpot.OK)
# saveRDS(lmr4gbpot.OxK,"models/lmr4gbpot.OxK.rds")
lmr4gbpot.OxK <- readRDS("models/lmr4gbpot.OxK.rds")


### study4_gb potency estimated marginal means
# d4gb_pot.emsumO <- summary(emmeans::emmeans(lmr4gbpot.i0, ~ outcome, pbkrtest.limit = 3180))
# saveRDS(d4gb_pot.emsumO,"models/d4gb_pot.emsumO.rds")
d4gb_pot.emsumO <- readRDS("models/d4gb_pot.emsumO.rds")
# d4gb_pot.empairO <- summary(pairs(emmeans(lmr4gbpot.i0, ~ outcome, pbkrtest.limit = 3180)))
# saveRDS(d4gb_pot.empairO,"models/d4gb_pot.empairO.rds")
d4gb_pot.empairO <- readRDS("models/d4gb_pot.empairO.rds")

# d4gb_pot.emsumK <- summary(emmeans::emmeans(lmr4gbpot.i0, ~ knowledge, pbkrtest.limit = 3180))
# saveRDS(d4gb_pot.emsumK,"models/d4gb_pot.emsumK.rds")
d4gb_pot.emsumK <- readRDS("models/d4gb_pot.emsumK.rds")
# d4gb_pot.empairK <- summary(pairs(emmeans(lmr4gbpot.i0, ~ knowledge, pbkrtest.limit = 3180)))
# saveRDS(d4gb_pot.empairK,"models/d4gb_pot.empairK.rds")
d4gb_pot.empairK <- readRDS("models/d4gb_pot.empairK.rds")

# d4gb_pot.emsumOK <- summary(emmeans::emmeans(lmr4gbpot.i0, ~ outcome*knowledge, pbkrtest.limit = 3180))
# saveRDS(d4gb_pot.emsumOK,"models/d4gb_pot.emsumOK.rds")
d4gb_pot.emsumOK <- readRDS("models/d4gb_pot.emsumOK.rds")
# d4gb_pot.empairOK <- summary(emmeans::emmeans(lmr4gbpot.i0, pairwise ~ outcome*knowledge, pbkrtest.limit = 3180))$contrasts
# saveRDS(d4gb_pot.empairOK,"models/d4gb_pot.empairOK.rds")
d4gb_pot.empairOK <- readRDS("models/d4gb_pot.empairOK.rds")


### d4a potency & study 2c causation

d4gb_pot <- d4gb %>% # same as above
  dplyr::select(-condCod) %>%
  pivot_wider(values_from = `CF responses`, names_from = "question") %>%
  mutate(Potency = as.numeric(Modal)*as.numeric(Counterfactual)) %>%
  drop_na()

d4_gbpotsum <- d4gb_pot %>%
  ...
  mutate(condition = case_when(
    outcome == "Improbable" ~ gsub("B", "U", condition),
    outcome == "Probable" ~ gsub("G", "P", condition),
    TRUE ~ condition))

#### merge summaries of potency and corresponding causal judgments
d2c.potsum <- d2c %>%   # 2*2*2 by 16 scenarios
  dplyr::select(cause, norm, outcome, knowledge, vign, condition) %>%
  group_by(knowledge,norm,outcome, vign, condition) %>% 
  summarise(N = length(cause),
            mean = mean(as.numeric(cause), na.rm=TRUE),
            sd = sd(as.numeric(cause),na.rm=TRUE),
            se = sd / sqrt(N)) %>%
  dplyr::rename(causation_N = N) %>%
  rename(causation_Mean = mean) %>%
  rename(causation_SD = sd) %>%
  rename(causation_SE = se) %>%
  rename(scenario = vign) 
d2c.potsum$outcome <- dplyr::recode(d2c.potsum$outcome, "Good Outcome"="Good", "Bad Outcome"="Bad")
d2c.potsum$norm <- dplyr::recode(as.factor(d2c.potsum$norm), "Moral Norm"="Morality", "Rational Norm"="Rationality")

d4gb.cspot_sum <- merge(d2c.potsum, d4_gbpotsum, by=c("knowledge", "norm", "outcome", "scenario", "condition")) # 128*13, combining 4a (d4gb) potency with 2c causation


#### d4a potency interaction: is there an interaction between potency and outcome valence in predicting causal judgments
##### (insignificant, 0.1916)
# lmr4a.potoutint0 <- lmer(causation_Mean ~ outcome*potencyMean + (1|scenario), data=d4gb.cspot_sum)
# lmr4a.potoutint1 <- lmer(causation_Mean ~ outcome + potencyMean + (1|scenario), data=d4gb.cspot_sum)
# lmr4a.potoutint <- anova(lmr4a.potoutint0, lmr4a.potoutint1)
# saveRDS(lmr4a.potoutint, "models/lmr4a.potoutint.rds")
lmr4a.potoutint <- readRDS("models/lmr4a.potoutint.rds")


#### d4a potency mediation: is potency a mediator between knowledge*outcome and causation?
# still very odd...
d4a.m <- d4gb.cspot_sum %>%
  mutate(Knowledge = factor(knowledge, levels = c("Ignorance","Knowledge"), labels = c(0, 1)),
          Outcome = factor(outcome, levels = c("Bad","Good"), labels = c(0, 1)),
          # recoding interaction term explicitly
          Interaction = case_when(Knowledge == 1 & Outcome == 0 ~ 0, ## Knowledge + Bad
            Knowledge == 1 & Outcome == 1 ~ 1, ## Knowledge + Good
            Knowledge == 0 & Outcome == 0 ~ 1, ## Ignorance + Bad
            Knowledge == 0 & Outcome == 1 ~ 0 ## Ignorance + Good
            ))
d4afull.fit <- lme4::lmer(causation_Mean ~ potencyMean + Knowledge + Outcome + Interaction + (1|scenario), data=d4a.m)
d4amed.fit <- lme4::lmer(potencyMean ~ Knowledge + Outcome + Interaction + (1|scenario), data=d4a.m)
d4amed.out <- mediate(d4afull.fit, d4amed.fit, sims=1000, treat="Interaction", mediator="potencyMean")
d4amed <- summary(d4amed.out)
...
We first looked into main effects of *Knowledge*, *Harm Type*, and *Outcome* on potency and we found statistical significance in both *Knowledge*, $\chi^2(1)$ = `r round(lmr4gbpot.knowledge$Chisq[2],digits=2)`, *p* `r round(lmr4gbpot.knowledge$'Pr(>Chisq)', 3)`, and *Outcome*, $\chi^2(1)$ = `r round(lmr4gbpot.outcome$Chisq[2],digits=2)`, *p* < .001. Potency being higher for knowledgeable agents (*M* = `r round(d4gb_pot.emsumK$emmean[2],digits=2)`, 95% CI = [`r round(d4gb_pot.emsumK$lower.CL[2],digits=2)`, `r round(d4gb_pot.emsumK$upper.CL[2],digits=2)`]) than ignorant agents (*M* = `r round(d4gb_pot.emsumK$emmean[1],digits=2)`, 95% CI = [`r round(d4gb_pot.emsumK$lower.CL[1],digits=2)`, `r round(d4gb_pot.emsumK$upper.CL[1],digits=2)`]) suggests that counterfactual reasoning exerts a larger amount of influence on causal selection on the former type of agents than the latter, *t*(`r round(d4gb_pot.empairK$df, digits=2)`) = `r round(d4gb_pot.empairK$t.ratio, 2)`, *p* < .001. Potency was also higher in bad outcomes (*M* = `r round(d4gb_pot.emsumO$emmean[1],digits=2)`, 95% CI = [`r round(d4gb_pot.emsumO$lower.CL[1],digits=2)`, `r round(d4gb_pot.emsumO$upper.CL[1],digits=2)`]) than in good outcomes (*M* = `r round(d4gb_pot.emsumO$emmean[2],digits=2)`, 95% CI = [`r round(d4gb_pot.emsumO$lower.CL[2],digits=2)`, `r sprintf("%.2f", d4gb_pot.emsumO$upper.CL[2])`]), *t*(`r round(d4gb_pot.empairO$df, digits=2)`) = `r round(d4gb_pot.empairO$t.ratio, 2)`, *p* `r ifelse(d4gb_pot.empairO$p.value < 0.001, "< .001", paste0("= ", round(d4gb_pot.empairO$p.value, 3)))`. In addition, there was a statistically significant *Knowledge* $\times$ *Outcome* interaction, $\chi^2(1)$ = `r round(lmr4gbpot.OxK$Chisq[2],digits=2)`, *p* = `r round(lmr4gbpot.OxK$'Pr(>Chisq)'[2],digits=3)`, such that in good outcomes, the effect of potency on knowledgeable agents (*M* = `r round(d4gb_pot.emsumOK$emmean[4],digits=2)`, 95% CI = [`r round(d4gb_pot.emsumOK$lower.CL[4],digits=2)`, `r round(d4gb_pot.emsumOK$upper.CL[4],digits=2)`]) did not differentiate greatly from that on ignorant agents (*M* = `r round(d4gb_pot.emsumOK$emmean[2],digits=2)`, 95% CI = [`r round(d4gb_pot.emsumOK$lower.CL[2],digits=2)`, `r round(d4gb_pot.emsumOK$upper.CL[2],digits=2)`]), *t*(`r round(d4gb_pot.empairOK$df[5])`) = `r round(d4gb_pot.empairOK$t.ratio[5], digits=2)`, *p* = `r round(d4gb_pot.empairOK$p.value[5], digits=3)`. Whereas in bad outcomes, potency had a much larger influence on agents with knowledge (*M* = `r round(d4gb_pot.emsumOK$emmean[3],digits=2)`, 95% CI = [`r round(d4gb_pot.emsumOK$lower.CL[3],digits=2)`, `r round(d4gb_pot.emsumOK$upper.CL[3],digits=2)`]) versus those without (*M* = `r round(d4gb_pot.emsumOK$emmean[1],digits=2)`, 95% CI = [`r round(d4gb_pot.emsumOK$lower.CL[1],digits=2)`, `r round(d4gb_pot.emsumOK$upper.CL[1],digits=2)`]), *t*(`r round(d4gb_pot.empairOK$df[2])`) = `r round(d4gb_pot.empairOK$t.ratio[2], digits=2)`, *p* `r round(d4gb_pot.empairOK$p.value[2], digits=3)`.

On the other hand, we did not see significant effects in *Harm Type*, $\chi^2(1)$ = `r round(lmr4gbpot.norm$Chisq[2],digits=2)`, *p* = `r round(lmr4gbpot.norm$'Pr(>Chisq)'[2], 3)`, or the three-way *Knowledge* $\times$ *Harm Type* $\times$ *Outcome* interaction, $\chi^2(1)$ = `r round(lmr4gbpot.3way$Chisq[2],digits=2)`, *p* = `r sprintf("%.3f",lmr4gbpot.3way$'Pr(>Chisq)'[2])`. We omitted interactions involving the *Harm Type* condition due to its minimal effect.



[[[To further corroborate the role of potency measure in causal cognition, we looked into its relationship with causal judgment collected in a similar study (i.e., Experiment 3a). Our analysis shows that potency is a mediator between *Knowledge*, *Outcome Normality* and *Causal Judgment*, (*ACME* = .034, *p* $<$ .001). We also found an interaction between potency and the normality of outcome in predicting causal judgments, $\chi^2(1)$ = `r round(lmr4a.potoutint$Chisq[2],digits=2)`, *p* = `r round(lmr4a.potoutint$'Pr(>Chisq)'[2],digits=3)`.]]]

```{r, 4a pot and cs-pot plots in case of use, fig.width=8.5, fig.height=7, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Participants' potency ratings regarding knowledgeable/ignorant agents who violated moral/rational norms when a separate outcome occurred that is good/bad."}

### d4a potency plots
d4gb.pot <- d4gb_pot %>%  # averaged across scenarios
...
d4gb.potfig <- d4gb.pot %>%
  ggplot(., aes(x=knowledge, y=potMean, fill=knowledge)) +
  geom_violin(alpha=.6, linewidth=.1) +
  scale_fill_manual(values=c("#bdd7e7", "#2171b5")) + ###bdd7e7 #6baed6
  geom_point(stat="identity",position=position_jitterdodge(jitter.width=.1,dodge.width=.9),alpha=.05, color="black") +
  geom_point(data=d4gb.pot.sum, aes(x=knowledge, y=potMean, fill=knowledge), shape=17, size=2,position=position_dodge(.9), color="black") +
  geom_line(data=d4gb.pot.sum, aes(x=knowledge, y=potMean, group=norm),size=1) +
  geom_errorbar(data=d4gb.pot.sum,aes(ymin=potMean-potsumSE, ymax=potMean+potsumSE), width=.2, position=position_dodge(.9),color="black") +
  facet_grid(outcome~norm, labeller = labeller(norm = c("Morality" = "Other-Harm", "Rationality" = "Self-Harm"), 
             outcome = c("Bad" = "Bad Outcome", "Good" = "Good Outcome"))) +
  ylab("Potency score") +
  xlab("") +
...
#ggsave("d4gb.potfig.pdf", width=8, height=12, path="figures")
d4gb.potfig

d4gb_cspot.plot <- d4gb.cspot_sum %>%
  dplyr::select(potencyMean, causation_Mean, knowledge, norm, outcome) %>%
  ggplot(aes(x=causation_Mean, y=potencyMean, color=knowledge)) +
  facet_grid(outcome~norm, labeller = labeller(norm = c("Morality" = "Other-Harm", "Rationality" = "Self-Harm"), 
             outcome = c("Bad" = "Bad Outcome", "Good" = "Good Outcome"))) +
  geom_point() +
...
Experiment 4a expands on our understanding of causal cognition in negative and positive secondary outcomes by examining components in counterfactual judgments, including the antecedent, the consequent, and their relationship (i.e., how participants assess the plausibility of alternative antecedent events and the likelihood of different outcomes under such hypothetical alternatives). In alignment with our hypothesis, modality ratings were influenced by both knowledge and outcome valence, suggesting that knowledgeable agents and negative secondary outcomes drove a stronger belief that the alternatives could be available. The consequent of alternative antecedents, on the other hand, was impacted by outcome valence and its interaction with knowledge, such that for negative outcomes, the counterfactual was more dependent on the antecedent when said antecedent involved a knowledgeable agent. Tying these effects back to previous studies on causal judgments—particularly the *Knowledge* $\times$ *Outcome* interaction in Experiment 2c—reveals that participants were more likely to consider an alternative outcome plausible when the causal agent was knowledgeable rather than ignorant, and this effect was especially pronounced in cases of unfavorable secondary outcomes, supporting the role of counterfactual reasoning in causal attribution.

We then put the two components of counterfactual representation together to understand whether and how they could jointly explain causal judgments, and saw in the unified potency results that it was impacted by knowledge, outcome valence, and the interaction between the two. Since this measure integrates perceived likelihood of counterfactual antecedent with the perceived likelihood of counterfactual consequent, the higher it is, the easier it should be for participants to think of such counterfactuals. Based on the observed effects, we conclude that knowledge status, outcome valence, and their interaction, all contribute to potency. More specifically, when causal agents acted knowingly as opposed to ignorantly or when secondary outcomes were negative as opposed to positive, counterfactual reasoning had a larger impact on causal judgments, and the said knowledge effect was even more prominent when secondary outcomes were unwelcome. It is also worth noting that potency did not interact with outcome valence or knowledge in predicting causal selection (?: we aren't doing interaction anymore so are these still worth including??), suggesting a unidirectional influence coming from outcome valence to potency, eventually to causal judgments. Taken together, our findings support a model where prescriptive normality influences causal judgments through counterfactual reasoning. 
...
In Experiment 4b, `r length(unique(d4.pb_demo$ResponseId))` participants were recruited (*M*~age~=`r round(mean(as.numeric(d4.pb_demo$age), na.rm=T),digits=2)`, *SD*~age~=`r round(sd(as.numeric(d4.pb_demo$age), na.rm=T),digits=2)`; `r length(which(d4.pb_demo$gender=="Female"))` females), among which `r sum(d4.pb_demo$ethn != "")` answered all our demographic questions. All participants were recruited through Amazon Mechanical Turk <http://www.mturk.com>.

```{r study4b dataset, echo=FALSE, warning=FALSE, message=FALSE}
d4pb <- read.csv("../data/d4b_data.csv", check.names = F) %>%
  mutate(`CF responses` = as.numeric(`CF responses`)) %>%
  mutate(outcome = case_when(outcome == "Abnormal" ~ "Improbable",
                             outcome == "Normal" ~ "Probable"))
```
...
```{r study4b md & cf interactions, echo=FALSE, warning=FALSE, message=FALSE}
## 4b Md main effects (knowledge significant <.001, outcome and norm insignificant)
# lmr4pb_md.main <- lmer(`CF responses` ~ knowledge + outcome + norm + (knowledge*outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Modal",])

# lmr4pb_md.outcome <- lmer(`CF responses` ~ knowledge + norm + (knowledge*outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Modal",])
# lmr4pb_md.outcome <- anova(lmr4pb_md.main,lmr4pb_md.outcome)
# saveRDS(lmr4pb_md.outcome,"models/lmr4pb_md.outcome.rds")
lmr4pb_md.outcome <- readRDS("models/lmr4pb_md.outcome.rds")

# lmr4pb_md.norm <- lmer(`CF responses` ~ knowledge + outcome + (knowledge*outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Modal",])
# lmr4pb_md.norm <- anova(lmr4pb_md.main,lmr4pb_md.norm)
# saveRDS(lmr4pb_md.norm,"models/lmr4pb_md.norm.rds")
lmr4pb_md.norm <- readRDS("models/lmr4pb_md.norm.rds")

# lmr4pb_md.knowledge <- lmer(`CF responses` ~ outcome + norm + (knowledge*outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Modal",])
# lmr4pb_md.knowledge <- anova(lmr4pb_md.main,lmr4pb_md.knowledge)
# saveRDS(lmr4pb_md.knowledge,"models/lmr4pb_md.knowledge.rds")
lmr4pb_md.knowledge <- readRDS("models/lmr4pb_md.knowledge.rds")


## Md: Three-way interaction (insignificant)
# lm4pb_md.0 <- lmer(`CF responses` ~ outcome*knowledge*norm + (knowledge*outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Modal",])
# lm4pb_md.1 <- lmer(`CF responses` ~ (knowledge*norm) + (outcome*knowledge) + (outcome*norm) + (knowledge*outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Modal",])
# lm4pb_md.3way <- anova(lm4pb_md.0, lm4pb_md.1)
# saveRDS(lm4pb_md.3way, "models/lm4pb_md.3way.rds")
lm4pb_md.3way <- readRDS("models/lm4pb_md.3way.rds")

## Md: Outcome x Knowledge interaction (insignificant)
# lm4pb_md.2 <- lmer(`CF responses` ~ (knowledge*norm) + (outcome*norm) + (knowledge*outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Modal",])
# lm4pb_md.OxK <- anova(lm4pb_md.1,lm4pb_md.2)
# saveRDS(lm4pb_md.OxK,"models/lm4pb_md.OxK.rds")
lm4pb_md.OxK <- readRDS("models/lm4pb_md.OxK.rds")

## Md: Outcome x Norm interaction (insignificant)
# lm4pb_md.3 <- lmer(`CF responses` ~ (knowledge*norm) + (outcome*knowledge) + (knowledge*outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Modal",])
# lm4pb_md.OxN <- anova(lm4pb_md.1,lm4pb_md.3)
# saveRDS(lm4pb_md.OxN,"models/lm4pb_md.OxN.rds")
lm4pb_md.OxN <- readRDS("models/lm4pb_md.OxN.rds")

## Md: Norm x Knowledge interaction (insignificant)
# lm4pb_md.4 <- lmer(`CF responses` ~ (knowledge*outcome) + (outcome*norm) + (knowledge*outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Modal",])
# lm4pb_md.NxK <- anova(lm4pb_md.1,lm4pb_md.4)
# saveRDS(lm4pb_md.NxK,"models/lm4pb_md.NxK.rds")
lm4pb_md.NxK <- readRDS("models/lm4pb_md.NxK.rds")

### Modality estimated marginal means
# d4pbmd.emsumK <- summary(emmeans::emmeans(lm4pb_md.0, ~ knowledge, pbkrtest.limit = 3226))
# saveRDS(d4pbmd.emsumK,"models/d4pbmd.emsumK.rds")
d4pbmd.emsumK <- readRDS("models/d4pbmd.emsumK.rds")
# d4pbmd.empairK <- summary(pairs(emmeans(lm4pb_md.0, ~ knowledge, pbkrtest.limit = 3226)))
# saveRDS(d4pbmd.empairK,"models/d4pbmd.empairK.rds")
d4pbmd.empairK <- readRDS("models/d4pbmd.empairK.rds")



## Cf main effects (knowledge significant .025, norm significant .05, outcome insignificant; used knowledge+outcome and optimizer for convergence)
# lmr4pb_cf.main <- lmer(`CF responses` ~ knowledge + outcome + norm + (knowledge+outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Counterfactual",],
#                        control = lmerControl(optimizer = "optimx", optCtrl = list(method = "nlminb")))
# 
# lmr4pb_cf.outcome <- lmer(`CF responses` ~ knowledge + norm + (knowledge+outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Counterfactual",],
#                           control = lmerControl(optimizer = "optimx", optCtrl = list(method = "nlminb")))
# lmr4pb_cf.outcome <- anova(lmr4pb_cf.main,lmr4pb_cf.outcome)
# saveRDS(lmr4pb_cf.outcome,"models/lmr4pb_cf.outcome.rds")
lmr4pb_cf.outcome <- readRDS("models/lmr4pb_cf.outcome.rds")

# lmr4pb_cf.norm <- lmer(`CF responses` ~ knowledge + outcome + (knowledge+outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Counterfactual",],
#                        control = lmerControl(optimizer = "optimx", optCtrl = list(method = "nlminb")))
# lmr4pb_cf.norm <- anova(lmr4pb_cf.main,lmr4pb_cf.norm)
# saveRDS(lmr4pb_cf.norm,"models/lmr4pb_cf.norm.rds")
lmr4pb_cf.norm <- readRDS("models/lmr4pb_cf.norm.rds")

# lmr4pb_cf.knowledge <- lmer(`CF responses` ~ outcome + norm + (knowledge+outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Counterfactual",],
#                             control = lmerControl(optimizer = "optimx", optCtrl = list(method = "nlminb")))
# lmr4pb_cf.knowledge <- anova(lmr4pb_cf.main,lmr4pb_cf.knowledge)
# saveRDS(lmr4pb_cf.knowledge,"models/lmr4pb_cf.knowledge.rds")
lmr4pb_cf.knowledge <- readRDS("models/lmr4pb_cf.knowledge.rds")


## Cf: Three-way interaction (insignificant; used knowledge+outcome and optimizer for convergence)
# lm4pb_cf.0 <- lmer(`CF responses` ~ outcome*knowledge*norm + (knowledge+outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Counterfactual",],
#                    control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
# lm4pb_cf.1 <- lmer(`CF responses` ~ (knowledge*norm) + (outcome*knowledge) + (outcome*norm) + (knowledge+outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Counterfactual",],
#                    control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
# lm4pb_cf.3way <- anova(lm4pb_cf.0, lm4pb_cf.1)
# saveRDS(lm4pb_cf.3way, "models/lm4pb_cf.3way.rds")
lm4pb_cf.3way <- readRDS("models/lm4pb_cf.3way.rds")

## Cf: Outcome x Knowledge interaction (insignificant)
# lm4pb_cf.2 <- lmer(`CF responses` ~ (knowledge*norm) + (outcome*norm) + (knowledge+outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Counterfactual",],
#                    control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
# lm4pb_cf.OxK <- anova(lm4pb_cf.1,lm4pb_cf.2)
# saveRDS(lm4pb_cf.OxK,"models/lm4pb_cf.OxK.rds")
lm4pb_cf.OxK <- readRDS("models/lm4pb_cf.OxK.rds")

## Cf: Outcome x Norm interaction (insignificant)
# lm4pb_cf.3 <- lmer(`CF responses` ~ (knowledge*norm) + (outcome*knowledge) + (knowledge+outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Counterfactual",],
#                    control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
# lm4pb_cf.OxN <- anova(lm4pb_cf.1,lm4pb_cf.3)
# saveRDS(lm4pb_cf.OxN,"models/lm4pb_cf.OxN.rds")
lm4pb_cf.OxN <- readRDS("models/lm4pb_cf.OxN.rds")

## Cf: Norm x Knowledge interaction (insignificant)
# lm4pb_cf.4 <- lmer(`CF responses` ~ (knowledge*outcome) + (outcome*norm) + (knowledge+outcome|scenario) + (1|ResponseId), data=d4pb[d4pb$question=="Counterfactual",],
#                    control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
# lm4pb_cf.NxK <- anova(lm4pb_cf.1,lm4pb_cf.4)
# saveRDS(lm4pb_cf.NxK,"models/lm4pb_cf.NxK.rds")
lm4pb_cf.NxK <- readRDS("models/lm4pb_cf.NxK.rds")


### Counterfactural estimated marginal means
# d4pbcf.emsumK <- summary(emmeans::emmeans(lm4pb_cf.0, ~ knowledge, pbkrtest.limit = 3227))
# saveRDS(d4pbcf.emsumK,"models/d4pbcf.emsumK.rds")
d4pbcf.emsumK <- readRDS("models/d4pbcf.emsumK.rds")
# d4pbcf.empairK <- summary(pairs(emmeans(lm4pb_cf.0, ~ knowledge, pbkrtest.limit = 3227)))
# saveRDS(d4pbcf.empairK,"models/d4pbcf.empairK.rds")
d4pbcf.empairK <- readRDS("models/d4pbcf.empairK.rds")

# d4pbcf.emsumN <- summary(emmeans::emmeans(lm4pb_cf.0, ~ norm, pbkrtest.limit = 3227))
# saveRDS(d4pbcf.emsumN,"models/d4pbcf.emsumN.rds")
d4pbcf.emsumN <- readRDS("models/d4pbcf.emsumN.rds")
# d4pbcf.empairN <- summary(pairs(emmeans(lm4pb_cf.0, ~ norm, pbkrtest.limit = 3327)))
# saveRDS(d4pbcf.empairN,"models/d4pbcf.empairN.rds")
d4pbcf.empairN <- readRDS("models/d4pbcf.empairN.rds")
...
We first looked at participants' modal ratings and found a significant main effect of *Knowledge*, $\chi^2(1)$ = `r round(lmr4pb_md.knowledge$Chisq[2], digits=2)`, *p* <.001, such that participants thought it was less plausible for ignorant agents to have not acted in the way they did (*M* = `r round(d4pbmd.emsumK$emmean[1],digits=2)`, 95% CI = [`r round(d4pbmd.emsumK$lower.CL[1], digits=2)`, `r round(d4pbmd.emsumK$upper.CL[1], digits=2)`]) than knowledgeable agents (*M* = `r round(d4pbmd.emsumK$emmean[2],digits=2)`, 95% CI = [`r round(d4pbmd.emsumK$lower.CL[2],digits=2)`, `r round(d4pbmd.emsumK$upper.CL[2],digits=2)`]), *t*(`r round(d4pbmd.empairK$df, digits=2)`) = `r round(d4pbmd.empairK$t.ratio[1], 2)`, *p* `r ifelse(d4pbmd.empairK$p.value < 0.001, "< .001", paste0("= ", round(d4pbmd.empairK$p.value, 3)))`. There was no significant main effect in *Outcome*, $\chi^2(1)$ = `r round(lmr4pb_md.outcome$Chisq[2],2)`, *p* = `r round(lmr4pb_md.outcome$'Pr(>Chisq)'[2],digits=3)`, or *Harm Type*, $\chi^2(1)$ = `r round(lmr4pb_md.norm$Chisq[2],digits=2)`, *p* = `r round(lmr4pb_md.norm$'Pr(>Chisq)'[2], 3)`. Analyses of interaction effects in modality ratings did not yield any significant results, including the *Knowledge* $\times$ *Harm Type* $\times$ *Outcome* interaction, $\chi^2(1)$ = `r round(lm4pb_md.3way$Chisq[2],digits=2)`, *p* = `r round(lm4pb_md.3way$'Pr(>Chisq)'[2],digits=3)`,  *Knowledge* $\times$ *Outcome* interaction, $\chi^2(1)$ = `r round(lm4pb_md.OxK$Chisq[2],digits=2)`, *p* = `r round(lm4pb_md.OxK$'Pr(>Chisq)'[2], digits=3)`. We omitted interactions involving the *Harm Type* condition due to its minimal effect.

Then, we explored counterfactual ratings, and discovered once again a significant main effect of *Knowledge*, $\chi^2(1)$ = `r round(lmr4pb_cf.knowledge$Chisq[2],digits=2)`, *p* = `r round(lmr4pb_cf.knowledge$'Pr(>Chisq)'[2],digits=3)`, such that participants were more inclined to believe that things could have been different if a knowledgeable agent had acted differently  (*M* = `r round(d4pbcf.emsumK$emmean[1],digits=2)`, 95% CI = [`r round(d4pbcf.emsumK$lower.CL[1],digits=2)`, `r round(d4pbcf.emsumK$upper.CL[1],digits=2)`]) than when an ignorant agent was involved (*M* = `r round(d4pbcf.emsumK$emmean[2],digits=2)`, 95% CI = [`r round(d4pbcf.emsumK$lower.CL[2],2)`, `r round(d4pbcf.emsumK$upper.CL[2],digits=2)`]), *t*(`r round(d4pbcf.empairK$df, digits=2)`) = `r round(d4pbcf.empairK$t.ratio[1], 2)`, *p* `r ifelse(d4pbcf.empairK$p.value < 0.001, "< .001", paste0("= ", round(d4pbcf.empairK$p.value, 3)))`. There was also a marginally significant effect of *Harm Type*, $\chi^2(1)$ = `r round(lmr4pb_cf.norm$Chisq[2],digits=2)`, *p* = `r round(lmr4pb_cf.norm$'Pr(>Chisq)'[2],digits=3)`: when an action harmed the agent themself, participants considered it more likely that the outcome could have been circumvented if the agent acted differently (*M* = `r round(d4pbcf.emsumN$emmean[2],digits=2)`, 95% CI = [`r round(d4pbcf.emsumN$lower.CL[2],2)`, `r round(d4pbcf.emsumN$upper.CL[2],digits=2)`]) than when the action brought harm to others (*M* = `r round(d4pbcf.emsumN$emmean[1],digits=2)`, 95% CI = [`r round(d4pbcf.emsumN$lower.CL[1],digits=2)`, `r round(d4pbcf.emsumN$upper.CL[1],digits=2)`]), *t*(`r round(d4pbcf.empairN$df, digits=2)`) = `r round(d4pbcf.empairN$t.ratio[1], 2)`, *p* `r ifelse(d4pbcf.empairN$p.value < 0.001, "< .001", paste0("= ", round(d4pbcf.empairN$p.value, 3)))`. There was no significant effect of *Outcome* in counterfactual judgments, $\chi^2(1)$ = `r round(lmr4pb_cf.outcome$Chisq[2],digits=2)`, *p* = `r ifelse(lmr4pb_cf.outcome$'Pr(>Chisq)' < 0.001, "< .001", paste0("= ", round(lmr4pb_cf.outcome$'Pr(>Chisq)', 3)))`. In addition, we did not find any interaction effects, including the three-way *Knowledge* $\times$ *Harm Type* $\times$ *Outcome* interaction effect, $\chi^2(1)$ = `r round(lm4pb_cf.3way$Chisq[2],digits=2)`, *p* = `r sprintf("%.3f", lm4pb_cf.3way$'Pr(>Chisq)'[2])` and the two-way *Knowledge* $\times$ *Outcome* interaction effect, $\chi^2(1)$ = `r round(lm4pb_cf.OxK$Chisq[2],digits=2)`, *p* = `r round(lm4pb_cf.OxK$'Pr(>Chisq)'[2], 3)`.



```{r, study4b plots, fig.width=8.5, fig.height=7, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Participants' modality ratings (top) and counterfactuality ratings (bottom) regarding knowledgeable/ignorant agents who violated moral/rational norms when a separate outcome occurred that is normal/abnormal."}
...
d4pb.modality <- ggplot(data=d4pb.md, aes(x=knowledge, y=mdMean, fill=knowledge)) +
  geom_violin(alpha=.6, linewidth=.1) +
  scale_fill_manual(values=c("#bdd7e7", "#2171b5")) + ###bdd7e7 #6baed6
  geom_point(stat="identity",position=position_jitterdodge(jitter.width=.1,dodge.width=.9),alpha=.05, color="black") +
  geom_point(data=d4pb.md.sum, aes(x=knowledge, y=mdMean, fill=knowledge), shape=17, size=2,position=position_dodge(.9), color="black") +
  geom_line(data=d4pb.md.sum, aes(x=knowledge, y=mdMean, group=norm),linewidth=1) +
  geom_errorbar(data=d4pb.md.sum,aes(ymin=mdMean-mdsumSE, ymax=mdMean+mdsumSE), width=.2, position=position_dodge(.9),color="black") +
  facet_grid(outcome~norm, labeller = labeller(norm = c("Morality" = "Other-Harm", "Rationality" = "Self-Harm"), 
             outcome = c("Probable" = "Probable Outcome", "Improbable" = "Improbable Outcome"))) +
  ylab("Modality rating") +
  xlab("") +
  theme_bw() +
  theme(
    ...
    axis.ticks = element_blank()) +
  scale_y_continuous(breaks=c(1, 3, 5, 7))
#ggsave("d4pb.modality.pdf", width=8, height=12, path="figures")
d4pb.modality


### d4b counterfactual plot
d4pb.cf <- d4pb %>%
  filter(question=="Counterfactual") %>%
  dplyr::select(ResponseId, `CF responses`, question, norm, outcome, knowledge) %>%
  group_by(`ResponseId`, norm, outcome, knowledge) %>%
...
d4pb.counterfactual <- ggplot(data=d4pb.cf, aes(x=knowledge, y=cfMean, fill=knowledge)) +
  geom_violin(alpha=.6, linewidth=.1) +
  scale_fill_manual(values=c("#bdd7e7", "#2171b5")) + ###bdd7e7 #6baed6
  geom_point(stat="identity",position=position_jitterdodge(jitter.width=.1,dodge.width=.9),alpha=.05, color="black") +
  geom_point(data=d4pb.cf.sum, aes(x=knowledge, y=cfMean, fill=knowledge), shape=17, size=2,position=position_dodge(.9), color="black") +
  geom_line(data=d4pb.cf.sum, aes(x=knowledge, y=cfMean, group=norm),size=1) +
  geom_errorbar(data=d4pb.cf.sum,aes(ymin=cfMean-cfsumSE, ymax=cfMean+cfsumSE), width=.2, position=position_dodge(.9),color="black") +
  facet_grid(outcome~norm, labeller = labeller(norm = c("Morality" = "Other-Harm", "Rationality" = "Self-Harm"), 
             outcome = c("Probable" = "Probable Outcome", "Improbable" = "Improbable Outcome"))) +
  ylab("Counterfactual rating") +
  xlab("") +
  ...
  theme(
    ...
    axis.ticks = element_blank()) +
  scale_y_continuous(breaks=c(1, 3, 5, 7))
#ggsave("d4pb.counterfactual.pdf", width=8, height=12, path="figures")
d4pb.counterfactual

```

```{r study4b potency & study3c causation, echo=FALSE, warning=FALSE, message=FALSE}
#Study 4b potency & causation

## 4b potency: for each condition in a scenario, use causal to predict cf*md --> create a new column potency
d4pb_pot <- d4pb %>%
  dplyr::select(-condCod) %>%
  pivot_wider(values_from = `CF responses`, names_from = "question") %>%
  mutate(Potency = as.numeric(Modal)*as.numeric(Counterfactual)) %>%
...
## main effects (knowledge significant <.001, outcome and norm insignificant; used knowledge+outcome and optimizer for convergence)
# lmr4pbpot.main <- lmer(Potency ~ knowledge + outcome + norm + (knowledge+outcome|scenario) + (1|ResponseId), data=d4pb_pot,
#                        control = lmerControl(optimizer = "bobyqa"))
# 
# lmr4pbpot.outcome <- lmer(Potency ~ knowledge + norm + (knowledge+outcome|scenario) + (1|ResponseId), data=d4pb_pot,
#                           control = lmerControl(optimizer = "bobyqa"))
# lmr4pbpot.outcome <- anova(lmr4pbpot.main,lmr4pbpot.outcome)
# saveRDS(lmr4pbpot.outcome,"models/lmr4pbpot.outcome.rds")
lmr4pbpot.outcome <- readRDS("models/lmr4pbpot.outcome.rds")

# lmr4pbpot.norm <- lmer(Potency ~ knowledge + outcome + (knowledge+outcome|scenario) + (1|ResponseId), data=d4pb_pot,
#                        control = lmerControl(optimizer = "bobyqa"))
# lmr4pbpot.norm <- anova(lmr4pbpot.main,lmr4pbpot.norm)
# saveRDS(lmr4pbpot.norm,"models/lmr4pbpot.norm.rds")
lmr4pbpot.norm <- readRDS("models/lmr4pbpot.norm.rds")

# lmr4pbpot.knowledge <- lmer(Potency ~ outcome + norm + (knowledge+outcome|scenario) + (1|ResponseId), data=d4pb_pot,
#                             control = lmerControl(optimizer = "bobyqa"))
# lmr4pbpot.knowledge <- anova(lmr4pbpot.main,lmr4pbpot.knowledge)
# saveRDS(lmr4pbpot.knowledge,"models/lmr4pbpot.knowledge.rds")
lmr4pbpot.knowledge <- readRDS("models/lmr4pbpot.knowledge.rds")


## three-way interaction (insignificant)
# lmr4pbpot.i0 <- lmer(Potency ~ knowledge * norm * outcome + (knowledge*outcome|scenario) + (1|ResponseId), data=d4pb_pot)
# lmr4pbpot.i1 <- lmer(Potency ~ (knowledge * norm) + (outcome * knowledge) + (outcome * norm) + (knowledge*outcome|scenario) + (1|ResponseId), data=d4pb_pot)
# lmr4pbpot.3way <- anova(lmr4pbpot.i0,lmr4pbpot.i1)
# saveRDS(lmr4pbpot.3way,"models/lmr4pbpot.3way.rds")
lmr4pbpot.3way <- readRDS("models/lmr4pbpot.3way.rds")


## two-way: Norm x Knowledge interaction (insignificant)
# lmr4pbpot.NK <- lmer(Potency ~ (knowledge * outcome) + (outcome * norm) + (knowledge*outcome|scenario) + (1|ResponseId), data=d4pb_pot)
# lmr4pbpot.NxK <- anova(lmr4pbpot.i1, lmr4pbpot.NK)
# saveRDS(lmr4pbpot.NxK,"models/lmr4pbpot.NxK.rds")
lmr4pbpot.NxK <- readRDS("models/lmr4pbpot.NxK.rds")

## two-way: Norm x Outcome interaction (insignificant)
# lmr4pbpot.NO <- lmer(Potency ~ (knowledge * outcome) + (knowledge * norm) + (knowledge*outcome|scenario) + (1|ResponseId), data=d4pb_pot)
# lmr4pbpot.NxO <- anova(lmr4pbpot.i1, lmr4pbpot.NO)
# saveRDS(lmr4pbpot.NxO,"models/lmr4pbpot.NxO.rds")
lmr4pbpot.NxO <- readRDS("models/lmr4pbpot.NxO.rds")

# two-way: Outcome x Knowledge interaction (insignificant)
# lmr4pbpot.OK <- lmer(Potency ~ (norm * outcome) + (knowledge * norm) + (knowledge*outcome|scenario) + (1|ResponseId), data=d4pb_pot)
# lmr4pbpot.OxK <- anova(lmr4pbpot.i1,lmr4pbpot.OK)
# saveRDS(lmr4pbpot.OxK,"models/lmr4pbpot.OxK.rds")
lmr4pbpot.OxK <- readRDS("models/lmr4pbpot.OxK.rds")

### study4_pb potency estimated marginal means

# d4pb_pot.emsumK <- summary(emmeans::emmeans(lmr4pbpot.i0, ~ knowledge, pbkrtest.limit = 3222))
# saveRDS(d4pb_pot.emsumK,"models/d4pb_pot.emsumK.rds")
d4pb_pot.emsumK <- readRDS("models/d4pb_pot.emsumK.rds")
# d4pb_pot.empairK <- summary(pairs(emmeans(lmr4pbpot.i0, ~ knowledge, pbkrtest.limit = 3222)))
# saveRDS(d4pb_pot.empairK,"models/d4pb_pot.empairK.rds")
d4pb_pot.empairK <- readRDS("models/d4pb_pot.empairK.rds")


### d4b potency & study 3a causation

d4pb_pot <- d4pb %>% # same as above
  dplyr::select(-condCod) %>%
  pivot_wider(values_from = `CF responses`, names_from = "question") %>%
  mutate(Potency = as.numeric(Modal)*as.numeric(Counterfactual)) %>%
  drop_na()

d4_pbpotsum <- d4pb_pot %>%
  dplyr::select(Potency, norm, outcome, knowledge, scenario, condition) %>%
  group_by(knowledge,norm,outcome,scenario,condition) %>% 
  summarise(N = length(Potency),
            mean = mean(as.numeric(Potency), na.rm=TRUE),
...
#### merge summaries of potency and corresponding causal judgments
d3a.potsum <- d3.cspb %>%   # 2*2*2 by 16 scenarios
  ...
  rename(causation_SD = sd) %>%
  rename(causation_SE = se)
d3a.potsum$outcome <- dplyr::recode(d3a.potsum$outcome, "Abnormal"="Improbable", "Normal"="Probable")

d4pb.cspot_sum <- merge(d3a.potsum, d4_pbpotsum, by=c("knowledge", "norm", "outcome", "scenario", "condition")) # 128*13, combining 4b (d4pb) potency with 3a causation

#### d4b potency interaction: is there an interaction between potency and outcome probability in predicting causal judgments
# ##### (significant, 0.02059)
# lmr4b.potoutint0 <- lmer(causation_Mean ~ outcome*potencyMean + (1|scenario), data=d4pb.cspot_sum)
# lmr4b.potoutint1 <- lmer(causation_Mean ~ outcome + potencyMean + (1|scenario), data=d4pb.cspot_sum)
# lmr4b.potoutint <- anova(lmr4b.potoutint0, lmr4b.potoutint1)
# saveRDS(lmr4b.potoutint, "models/lmr4b.potoutint.rds")
lmr4b.potoutint <- readRDS("models/lmr4b.potoutint.rds")


#### d4b potency mediation: is potency a mediator between (just) knowledge and causation?
# still weird in terms of identical results across levels...
d4b.m <- d4pb.cspot_sum %>%
  mutate(Knowledge = factor(knowledge, levels = c("Ignorance","Knowledge"), labels = c(0, 1)),
         Outcome = factor(outcome, levels = c("Improbable","Probable"), labels = c(0, 1)),
         # recoding interaction term explicitly
         Interaction = case_when(Knowledge == 1 & Outcome == 0 ~ 0, ## Knowledge + Improbable
                                 Knowledge == 1 & Outcome == 1 ~ 1, ## Knowledge + Probable
                                 Knowledge == 0 & Outcome == 0 ~ 1, ## Ignorance + Improbable
                                 Knowledge == 0 & Outcome == 1 ~ 0 ## Ignorance + Probable
         ))
d4bfull.fit <- lme4::lmer(causation_Mean ~ potencyMean + Knowledge + (1|scenario), data=d4b.m)
d4bmed.fit <- lme4::lmer(potencyMean ~ Knowledge + (1|scenario), data=d4b.m)
d4bmed.out <- mediate(d4bfull.fit, d4bmed.fit, sims=1000, treat="Knowledge", mediator="potencyMean")
d4bmed <- summary(d4bmed.out)
...
Examining main effects of *Knowledge*, *Harm Type*, and *Outcome* on potency, we found statistical significance only in *Knowledge*, $\chi^2(1)$ = `r round(lmr4pbpot.knowledge$Chisq[2],digits=2)`, *p* `r ifelse(lmr4pbpot.knowledge$'Pr(>Chisq)' < 0.001, "< .001", paste0("= ", round(lmr4pbpot.knowledge$'Pr(>Chisq)', 3)))`, which is to say that counterfactual reasoning has a bigger impact on causal judgment when the agents involved knew the consequence of their actions (*M* = `r round(d4pb_pot.emsumK$emmean[2],digits=2)`, 95% CI = [`r round(d4pb_pot.emsumK$lower.CL[2],digits=2)`, `r round(d4pb_pot.emsumK$upper.CL[2],digits=2)`]) than when they did not (*M* = `r round(d4pb_pot.emsumK$emmean[1],digits=2)`, 95% CI = [`r round(d4pb_pot.emsumK$lower.CL[1],digits=2)`, `r round(d4pb_pot.emsumK$upper.CL[1],digits=2)`]), *t*(`r round(d4pb_pot.empairK$df, digits=2)`) = `r round(d4pb_pot.empairK$t.ratio, 2)`, *p* `r ifelse(d4pb_pot.empairK$p.value < 0.001, "< .001", paste0("= ", round(d4pb_pot.empairK$p.value, 3)))`. Other than that, we did not see a main effect in either *Outcome*, $\chi^2(1)$ = `r round(lmr4pbpot.outcome$Chisq[2],digits=2)`, *p* `r ifelse(lmr4pbpot.outcome$'Pr(>Chisq)' < 0.001, "< .001", paste0("= ", round(lmr4pbpot.outcome$'Pr(>Chisq)', 3)))`, or *Harm Type*, $\chi^2(1)$ = `r round(lmr4pbpot.norm$Chisq[2],digits=2)`, *p* `r ifelse(lmr4pbpot.norm$'Pr(>Chisq)' < 0.001, "< .001", paste0("= ", round(lmr4pbpot.norm$'Pr(>Chisq)', 3)))`. No significant interaction effects were observed, including the three-way *Knowledge* $\times$ *Harm Type* $\times$ *Outcome* interaction, $\chi^2(1)$ = `r round(lmr4pbpot.3way$Chisq[2],digits=2)`, *p* = `r sprintf("%.3f",lmr4pbpot.3way$'Pr(>Chisq)'[2])`,  and the *Knowledge* $\times$ *Outcome* interaction, $\chi^2(1)$ = `r round(lmr4pbpot.OxK$Chisq[2],digits=2)`, *p* = `r round(lmr4pbpot.OxK$'Pr(>Chisq)'[2],digits=3)`. We omitted interactions involving the *Harm Type* condition due to its minimal effect.


[[[To further corroborate the role of potency measure in causal cognition, we looked into its relationship with causal judgment collected in a similar study (i.e., Experiment 3a). Our analysis shows that potency is a mediator between *Knowledge*, *Outcome Normality* and *Causal Judgment*, (*ACME* = .034, *p* $<$ .001???). We also found an interaction between potency and the probability of outcome in predicting causal judgments, $\chi^2(1)$ = `r round(lmr4b.potoutint$Chisq[2],digits=2)`, *p* = `r round(lmr4b.potoutint$'Pr(>Chisq)'[2],digits=3)`.]]]


```{r, 4b pot and cspot plots in case of use, fig.width=8.5, fig.height=7, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Participants' potency ratings regarding knowledgeable/ignorant agents who violated moral/rational norms when a separate outcome occurred that is normal/abnormal."}

### d4b potency plots
d4pb.pot <- d4pb_pot  %>%   # averaged across scenarios
  dplyr::select(ResponseId, Potency, norm, outcome, knowledge) %>%
  group_by(`ResponseId`, norm, outcome, knowledge) %>%
  summarise(N = length(Potency),
            mean = mean(as.numeric(Potency), na.rm=TRUE)) %>%
...
d4pb.potfig <- d4pb.pot %>%
  ggplot(., aes(x=knowledge, y=potMean, fill=knowledge)) +
  geom_violin(alpha=.6, linewidth=.1) +
  scale_fill_manual(values=c("#bdd7e7", "#2171b5")) + ###bdd7e7 #6baed6
  geom_point(stat="identity",position=position_jitterdodge(jitter.width=.1,dodge.width=.9),alpha=.05, color="black") +
  geom_point(data=d4pb.pot.sum, aes(x=knowledge, y=potMean, fill=knowledge), shape=17, size=2,position=position_dodge(.9), color="black") +
  geom_line(data=d4pb.pot.sum, aes(x=knowledge, y=potMean, group=norm),size=1) +
  geom_errorbar(data=d4pb.pot.sum,aes(ymin=potMean-potsumSE, ymax=potMean+potsumSE), width=.2, position=position_dodge(.9),color="black") +
  facet_grid(outcome~norm, labeller = labeller(norm = c("Morality" = "Other-Harm", "Rationality" = "Self-Harm"), 
             outcome = c("Probable" = "Probable Outcome", "Improbable" = "Improbable Outcome"))) +
  ylab("Potency score") +
  xlab("") +
  ...
  theme(
    ...
    axis.ticks = element_blank()) +
  scale_y_continuous(breaks=c(7,21,35,49))
#ggsave("d4pb.potfig.pdf", width=8, height=12, path="figures")
d4pb.potfig


d4pb_cspot.plot <- d4pb.cspot_sum %>%
  dplyr::select(potencyMean, causation_Mean, knowledge, norm, outcome) %>%
  ggplot(aes(x=causation_Mean, y=potencyMean, color=knowledge)) +
  facet_grid(outcome~norm, labeller = labeller(norm = c("Morality" = "Other-Harm", "Rationality" = "Self-Harm"), 
             outcome = c("Probable" = "Probable Outcome", "Improbable" = "Improbable Outcome"))) +
