...
author: "| Songzhi Wu, Catherine Holland, Tobias Gerstenberg, Liane Young, and Jonathan Phillips"
date: "2024"
output:
  pdf_document:
    fig_caption: true
...
d1demo <- read.csv("../data/_study1/d1_demo.csv") 
d1demo$age[d1demo$age==1993] <- NA ## one person entered their age as 1993 (presumably the year that they were born), but excluding here bc not sure.
...
d1 <- read.csv("../data/_study1/d1_data.csv")
...
# saveRDS(lmr1.i,"models/_study1/lmr1i.rds")
lmr1.i <- readRDS("models/_study1/lmr1i.rds")
...
# saveRDS(lmr1.a,"models/_study1/lmr1a.rds")
lmr1.a <- readRDS("models/_study1/lmr1a.rds")

### Main effect of outcome (significant: p = 0.007602)
# lmr1.3 <- lmer(Cause ~ Agent + (Agent*Outcome|Vignette) + (1|Subj), data=d1,
#                control = lmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')))
# lmr1.o <- anova(lmr1.1,lmr1.3)
# saveRDS(lmr1.o,"models/_study1/lmr1o.rds")
lmr1.o <- readRDS("models/_study1/lmr1o.rds")
...
# saveRDS(d1.emsumAO,"models/_study1/d1.emsumAO.rds")
d1.emsumAO <- readRDS("models/_study1/d1.emsumAO.rds")
# d1.empairAO <- summary(emmeans::emmeans(lmr1.0, pairwise ~ Agent*Outcome))$contrasts
# saveRDS(d1.empairAO,"models/_study1/d1.empairAO.rds")
d1.empairAO <- readRDS("models/_study1/d1.empairAO.rds")

#Effect of Agent:
# d1.emsumA <- summary(emmeans::emmeans(lmr1.0, ~ Agent))
# saveRDS(d1.emsumA,"models/_study1/d1.emsumA.rds")
d1.emsumA <- readRDS("models/_study1/d1.emsumA.rds")
# d1.empairA <- summary(pairs(emmeans(lmr1.0, ~ Agent)))
# saveRDS(d1.empairA,"models/_study1/d1.empairA.rds")
d1.empairA <- readRDS("models/_study1/d1.empairA.rds")

#Effect of Outcome:
# d1.emsumO <- summary(emmeans::emmeans(lmr1.0, ~ Outcome))
# saveRDS(d1.emsumO,"models/_study1/d1.emsumO.rds")
d1.emsumO <- readRDS("models/_study1/d1.emsumO.rds")
# d1.empairO <- summary(pairs(emmeans(lmr1.0, ~ Outcome)))
# saveRDS(d1.empairO,"models/_study1/d1.empairO.rds")
d1.empairO <- readRDS("models/_study1/d1.empairO.rds")
...
We further decomposed this interaction and found that when agents knew about the outcome that may come as a consequence of their action, they were judged as much more causal for bad outcomes (*M* = `r round(d1.emsumAO$emmean[6],digits=2)`, 95% CI = [`r round(d1.emsumAO$lower.CL[6],digits=2)`, `r round(d1.emsumAO$upper.CL[6],digits=2)`]) than for good outcomes (*M* = `r round(d1.emsumAO$emmean[3],digits=2)`, 95% CI = [`r round(d1.emsumAO$lower.CL[3],digits=2)`, `r round(d1.emsumAO$upper.CL[3],digits=2)`]), *t*(`r round(d1.empairAO$df[12])`) = `r round(d1.empairAO$t.ratio[12], digits=2)`, *p* <.001. In contrast, when agents were oblivious about the outcome, they were not judged to be much more causal for bad outcomes (*M* = `r round(d1.emsumAO$emmean[5],digits=2)`, 95% CI = [`r round(d1.emsumAO$lower.CL[5],digits=2)`, `r round(d1.emsumAO$upper.CL[5],digits=2)`]) than good outcomes (*M* = `r round(d1.emsumAO$emmean[2],digits=2)`, 95% CI = [`r round(d1.emsumAO$lower.CL[2],digits=2)`, `r round(d1.emsumAO$upper.CL[2],digits=2)`]), *t*(`r round(d1.empairAO$df[8])`) = `r round(d1.empairAO$t.ratio[8], digits=2)`, *p* = `r round(d1.empairAO$p.value[8], digits=3)`. Similarly, causal judgments about non-agentic objects did not differentiate much between bad outcomes (*M* = `r round(d1.emsumAO$emmean[4],digits=2)`, 95% CI = [`r round(d1.emsumAO$lower.CL[4],digits=2)`, `r round(d1.emsumAO$upper.CL[4],digits=2)`]) and good outcomes (*M* = `r round(d1.emsumAO$emmean[1],digits=2)`, 95% CI = [`r round(d1.emsumAO$lower.CL[1],digits=2)`, `r round(d1.emsumAO$upper.CL[1],digits=2)`]), *t*(`r round(d1.empairAO$df[3])`) = `r round(d1.empairAO$t.ratio[3], digits=2)`, *p* = `r round(d1.empairAO$p.value[3], digits=3)`. In short, the valence of the outcome only affected participants' causal judgments when the agent at the beginning of the causal chain *knew* about the valence of the outcome (see *Figure* 2).
...
# saveRDS(glmr1.i,"models/_study1/glmr1i.rds")
glmr1.i <- readRDS("models/_study1/glmr1i.rds")

## Main effect of agent (significant: p <.001)
# glmr1.2 <- glmer(Counterfactual ~ Outcome + (Agent*Outcome|Vignette) + (1|Subj), data=d1, family = "binomial",
#                  control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
# glmr1.a <- anova(glmr1.1,glmr1.2)
# saveRDS(glmr1.a,"models/_study1/glmr1a.rds")
glmr1.a <- readRDS("models/_study1/glmr1a.rds")

# Main effect of outcome (significant: p = 0.00119)
# glmr1.3 <- glmer(Counterfactual ~ Agent + (Agent*Outcome|Vignette) + (1|Subj), data=d1, family = "binomial",
#                  control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
# glmr1.o <- anova(glmr1.1,glmr1.3)
# saveRDS(glmr1.o,"models/_study1/glmr1o.rds")
glmr1.o <- readRDS("models/_study1/glmr1o.rds")

### d1 counterfactual estimated marginal means & proportions

## Interaction effect:
# d1.cfsumAO <- summary(emmeans::emmeans(glmr1.0, ~ Agent*Outcome))
# saveRDS(d1.cfsumAO,"models/_study1/d1.cfsumAO.rds")
d1.cfsumAO <- readRDS("models/_study1/d1.cfsumAO.rds")
# d1.cfpairAO <- summary(pairs(emmeans::emmeans(glmr1.0, ~ Agent*Outcome)))
# saveRDS(d1.cfpairAO,"models/_study1/d1.cfpairAO.rds")
d1.cfpairAO <- readRDS("models/_study1/d1.cfpairAO.rds")

## Effect of Agent
# d1.cfsumA <- summary(emmeans::emmeans(glmr1.0, ~ Agent))
# saveRDS(d1.cfsumA,"models/_study1/d1.cfsumA.rds")
d1.cfsumA <- readRDS("models/_study1/d1.cfsumA.rds")
# d1.cfpairA <- summary(pairs(emmeans(glmr1.0, ~ Agent)))
# saveRDS(d1.cfpairA,"models/_study1/d1.cfpairA.rds")
d1.cfpairA <- readRDS("models/_study1/d1.cfpairA.rds")

## Effect of Outcome
# d1.cfsumO <- summary(emmeans::emmeans(glmr1.0, ~ Outcome))
# saveRDS(d1.cfsumO,"models/_study1/d1.cfsumO.rds")
d1.cfsumO <- readRDS("models/_study1/d1.cfsumO.rds")
# d1.cfpairO <- summary(pairs(emmeans(glmr1.0, ~ Outcome)))
# saveRDS(d1.cfpairO,"models/_study1/d1.cfpairO.rds")
d1.cfpairO <- readRDS("models/_study1/d1.cfpairO.rds")
...
## ERA - update the colors below to match the above graphs!!
...
[restate what we did here as you did in the discussion of 3b] At a broad, the results from this experiment verified that participants perceived the bad secondary outcomes as more negatively valenced than the good secondary outcomes. Additionally, we also observed that the bad outcomes were perceived as less normal than the good outcomes. This finding supports the suggestion that perceptions of normality are impacted by both descriptive information (the probability of the outcomes) prescriptive information (the valence of the outcomes). In short, we found that, in support of the more parsimonious account, varying outcome valence changes not only the perceived goodness but perceived normality.
...
Experiment 4 expands on our understanding of causal cognition by examining modal and counterfactual judgments, that is, how participants assess the plausibility of alternative antecedent events and the likelihood of different outcomes under such hypothetical alternatives. 

[We should start by talking about the effect of knowledge, and then also say that interesting it was somewhat affected by outcome normality. But no interaction. Iterim summary: this suggests that the availability of alternatives was primarily driven by whether the agent knowingly cauased harm. 

Switch to counterfactual judgments: Main effect of outcome, but no effect of knowledge. Interestingly we also found a small but significant interaction effect, between outcome and knowledge, so that only for abnormal outcomes, participants thought that the outcome depended more on the antecedent event when that antecedent event involved the agent knowingly causing harm.

Critically, we put these two pieces together to ask whether they could jointly explain causal judgments, and more specifically the strong and persistent interaction between knowledge and outcome normality. This is the counterfactual potency measure... ]

Consistent with previous studies, we found that outcome normality played a crucial role in shaping participants' judgments, such that when secondary outcomes were abnormal (i.e., bad or improbable), participants were less inclined to believe that agents could have acted differently or that alternative consequences could have occurred if antecedent conditions had been altered. Importantly, we also observed a *Knowledge* $\times$ *Outcome* interaction in counterfactual judgment which revealed that participants were more likely to consider an alternative outcome plausible when the causal agent was knowledgeable rather than ignorant, especially in cases of abnormal outcomes. Tying this effect back to the same *Knowledge* $\times$ *Outcome* interaction previously found studies involving causal judgments prompted us to consider the underlying role of counterfactual reasoning in causal attribution. To explore this link, we probed potency, a concept intended to capture the combined impact of the perceived likelihood of counterfactual antecedent and the perceived likelihood of counterfactual consequence on causal judgments. This combined measure speaks to both the antecedent and the consequent of counterfactual alternatives, and the higher it is, the easier it should be for participants to think of such counterfactuals. Based on the results, we conclude that knowledge status, outcome normality, and their interaction, all contribute to potency. More specifically, when causal agents acted knowingly as opposed to ignorantly or when secondary outcomes were abnormal as opposed to normal, counterfactual reasoning had a larger impact on causal judgments, and the said knowledge effect was even more prominent when secondary outcomes were abnormal. It is also worth noting that potency did not interact with outcome probability in predicting causal selection, suggesting a unidirectional influence coming from outcome probability to potency, to causal judgments (it's just probability, not normality though...?). Taken together, our findings support a model where normality influences causal judgments through counterfactual reasoning. 
